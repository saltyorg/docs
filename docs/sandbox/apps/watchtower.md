---
hide:
  - tags
tags:
  - watchtower
  - monitoring
  - docker
---

# Watchtower

## Overview

[Watchtower](https://containrrr.dev/watchtower/) is a process for automating Docker container base image updates.

With watchtower you can update the running version of your containerized app simply by pushing a new image to the Docker Hub or your own image registry. Watchtower will pull down your new image, gracefully shut down your existing container and restart it with the same options that were used when it was deployed initially.

| Details     |             |             |             |
|-------------|-------------|-------------|-------------|
| [:material-home: Project home](https://containrrr.dev/watchtower/){: .header-icons } | [:octicons-link-16: Docs](https://containrrr.github.io/watchtower){: .header-icons } | [:octicons-mark-github-16: Github](https://github.com/containrrr/watchtower){: .header-icons } | [:material-docker: Docker](https://hub.docker.com/r/containrrr/watchtower){: .header-icons }|

### 1. Installation

``` shell

sb install sandbox-watchtower

```

<!-- BEGIN SALTBOX MANAGED VARIABLES SECTION -->
<!-- This section is managed by saltbox/test.py - DO NOT EDIT MANUALLY -->
## Role Defaults

!!! info
    Variables can be overridden in `/srv/git/saltbox/inventories/host_vars/localhost.yml`.

    This role supports multiple instances via `watchtower_instances`.

    ```yaml { .sb-show-on-unchecked title="Applies to all instances of watchtower:" }
    watchtower_role_web_subdomain: "custom"
    ```

    ```yaml { .sb-show-on-checked title="Applies to a specific instance (e.g., `watchtower2`):" }
    watchtower2_web_subdomain: "custom2"
    ```

<label class="md-button md-button--stretch" for="sb-checkbox--var-level">
   <input type="checkbox" id="sb-checkbox--var-level"><span class="sb-show-on-unchecked">Show instance-level variables</span><span class="sb-show-on-checked">Show role-level variables</span>
</label>

??? warning "Avoid overriding variables ending in `_default`"

    When overriding variables that end in `_default` (like `watchtower_docker_envs_default`), you replace the entire default configuration. Future updates that add new default values will not be applied to your setup, potentially breaking functionality.

    Instead, use the corresponding `_custom` variable (like `watchtower_docker_envs_custom`) to add your changes. Custom values are merged with defaults, ensuring you receive updates.

=== "Basics"

    ??? variable list "`watchtower_instances`"

        ```yaml
        # Type: list
        watchtower_instances: ["watchtower"]
        ```

        !!! example "Example Override"

            ```yaml
            # Type: list
            watchtower_instances: ["watchtower", "watchtower2"]
            ```

=== "Settings"

    ??? variable bool "`watchtower_role_metrics_enable`{ .sb-show-on-unchecked }`watchtower2_metrics_enable`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: bool (true/false)
        watchtower_role_metrics_enable: false
        ```

        ```yaml { .sb-show-on-checked }
        # Type: bool (true/false)
        watchtower2_metrics_enable: false
        ```

    ??? variable bool "`watchtower_role_metrics_external`{ .sb-show-on-unchecked }`watchtower2_metrics_external`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: bool (true/false)
        watchtower_role_metrics_external: false
        ```

        ```yaml { .sb-show-on-checked }
        # Type: bool (true/false)
        watchtower2_metrics_external: false
        ```

    ??? variable string "`watchtower_role_poll_interval`{ .sb-show-on-unchecked }`watchtower2_poll_interval`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_poll_interval: "43200"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_poll_interval: "43200"
        ```

=== "Paths"

    ??? variable string "`watchtower_role_paths_folder`{ .sb-show-on-unchecked }`watchtower2_paths_folder`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_paths_folder: "{{ watchtower_name }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_paths_folder: "{{ watchtower_name }}"
        ```

    ??? variable string "`watchtower_role_paths_location`{ .sb-show-on-unchecked }`watchtower2_paths_location`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_paths_location: "{{ server_appdata_path }}/{{ watchtower_role_paths_folder }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_paths_location: "{{ server_appdata_path }}/{{ watchtower_role_paths_folder }}"
        ```

=== "Web"

    ??? variable string "`watchtower_role_web_subdomain`{ .sb-show-on-unchecked }`watchtower2_web_subdomain`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_web_subdomain: "{{ watchtower_name }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_web_subdomain: "{{ watchtower_name }}"
        ```

    ??? variable string "`watchtower_role_web_domain`{ .sb-show-on-unchecked }`watchtower2_web_domain`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_web_domain: "{{ user.domain }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_web_domain: "{{ user.domain }}"
        ```

    ??? variable string "`watchtower_role_web_port`{ .sb-show-on-unchecked }`watchtower2_web_port`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_web_port: "8080"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_web_port: "8080"
        ```

    ??? variable string "`watchtower_host`{ .sb-show-on-unchecked }`watchtower_host`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_host: "{{ lookup('role_var', '_web_subdomain', role='watchtower') + '.' + lookup('role_var', '_web_domain', role='watchtower') }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower_host: "{{ lookup('role_var', '_web_subdomain', role='watchtower') + '.' + lookup('role_var', '_web_domain', role='watchtower') }}"
        ```

=== "DNS"

    ??? variable string "`watchtower_role_dns_record`{ .sb-show-on-unchecked }`watchtower2_dns_record`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_dns_record: "{{ lookup('role_var', '_web_subdomain', role='watchtower') }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_dns_record: "{{ lookup('role_var', '_web_subdomain', role='watchtower') }}"
        ```

    ??? variable string "`watchtower_role_dns_zone`{ .sb-show-on-unchecked }`watchtower2_dns_zone`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_dns_zone: "{{ lookup('role_var', '_web_domain', role='watchtower') }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_dns_zone: "{{ lookup('role_var', '_web_domain', role='watchtower') }}"
        ```

    ??? variable bool "`watchtower_role_dns_proxy`{ .sb-show-on-unchecked }`watchtower2_dns_proxy`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: bool (true/false)
        watchtower_role_dns_proxy: "{{ dns_proxied }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: bool (true/false)
        watchtower2_dns_proxy: "{{ dns_proxied }}"
        ```

=== "Traefik"

    ??? variable string "`watchtower_role_traefik_sso_middleware`{ .sb-show-on-unchecked }`watchtower2_traefik_sso_middleware`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_traefik_sso_middleware: ""
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_traefik_sso_middleware: ""
        ```

    ??? variable string "`watchtower_role_traefik_middleware_default`{ .sb-show-on-unchecked }`watchtower2_traefik_middleware_default`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_traefik_middleware_default: "{{ traefik_default_middleware }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_traefik_middleware_default: "{{ traefik_default_middleware }}"
        ```

    ??? variable string "`watchtower_role_traefik_middleware_custom`{ .sb-show-on-unchecked }`watchtower2_traefik_middleware_custom`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_traefik_middleware_custom: ""
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_traefik_middleware_custom: ""
        ```

    ??? variable string "`watchtower_role_traefik_certresolver`{ .sb-show-on-unchecked }`watchtower2_traefik_certresolver`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_traefik_certresolver: "{{ traefik_default_certresolver }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_traefik_certresolver: "{{ traefik_default_certresolver }}"
        ```

    ??? variable string "`watchtower_role_traefik_enabled`{ .sb-show-on-unchecked }`watchtower2_traefik_enabled`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_traefik_enabled: "{{ true
                                          if (lookup('role_var', '_metrics_enable', role='watchtower') and lookup('role_var', '_metrics_external', role='watchtower'))
                                          else false }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_traefik_enabled: "{{ true
                                      if (lookup('role_var', '_metrics_enable', role='watchtower') and lookup('role_var', '_metrics_external', role='watchtower'))
                                      else false }}"
        ```

    ??? variable bool "`watchtower_role_traefik_api_enabled`{ .sb-show-on-unchecked }`watchtower2_traefik_api_enabled`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: bool (true/false)
        watchtower_role_traefik_api_enabled: false
        ```

        ```yaml { .sb-show-on-checked }
        # Type: bool (true/false)
        watchtower2_traefik_api_enabled: false
        ```

    ??? variable string "`watchtower_role_traefik_api_endpoint`{ .sb-show-on-unchecked }`watchtower2_traefik_api_endpoint`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_traefik_api_endpoint: ""
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_traefik_api_endpoint: ""
        ```

=== "Docker"

    <h5>Container</h5>

    ??? variable string "`watchtower_role_docker_container`{ .sb-show-on-unchecked }`watchtower2_docker_container`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_docker_container: "{{ watchtower_name }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_docker_container: "{{ watchtower_name }}"
        ```

    <h5>Image</h5>

    ??? variable bool "`watchtower_role_docker_image_pull`{ .sb-show-on-unchecked }`watchtower2_docker_image_pull`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: bool (true/false)
        watchtower_role_docker_image_pull: true
        ```

        ```yaml { .sb-show-on-checked }
        # Type: bool (true/false)
        watchtower2_docker_image_pull: true
        ```

    ??? variable string "`watchtower_role_docker_image_repo`{ .sb-show-on-unchecked }`watchtower2_docker_image_repo`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_docker_image_repo: "containrrr/watchtower"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_docker_image_repo: "containrrr/watchtower"
        ```

    ??? variable string "`watchtower_role_docker_image_tag`{ .sb-show-on-unchecked }`watchtower2_docker_image_tag`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_docker_image_tag: "latest"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_docker_image_tag: "latest"
        ```

    ??? variable string "`watchtower_role_docker_image`{ .sb-show-on-unchecked }`watchtower2_docker_image`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_docker_image: "{{ lookup('role_var', '_docker_image_repo', role='watchtower') }}:{{ lookup('role_var', '_docker_image_tag', role='watchtower') }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_docker_image: "{{ lookup('role_var', '_docker_image_repo', role='watchtower') }}:{{ lookup('role_var', '_docker_image_tag', role='watchtower') }}"
        ```

    <h5>Envs</h5>

    ??? variable dict "`watchtower_role_docker_envs_default`{ .sb-show-on-unchecked }`watchtower2_docker_envs_default`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: dict
        watchtower_role_docker_envs_default: 
          TZ: "{{ tz }}"
          WATCHTOWER_CLEANUP: "true"
          WATCHTOWER_POLL_INTERVAL: "{{ lookup('role_var', '_poll_interval', role='watchtower') }}"
          WATCHTOWER_HTTP_API_METRICS: "{{ 'true'
                                        if lookup('role_var', '_metrics_enable', role='watchtower')
                                        else omit }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: dict
        watchtower2_docker_envs_default: 
          TZ: "{{ tz }}"
          WATCHTOWER_CLEANUP: "true"
          WATCHTOWER_POLL_INTERVAL: "{{ lookup('role_var', '_poll_interval', role='watchtower') }}"
          WATCHTOWER_HTTP_API_METRICS: "{{ 'true'
                                        if lookup('role_var', '_metrics_enable', role='watchtower')
                                        else omit }}"
        ```

    ??? variable dict "`watchtower_role_docker_envs_custom`{ .sb-show-on-unchecked }`watchtower2_docker_envs_custom`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: dict
        watchtower_role_docker_envs_custom: {}
        ```

        ```yaml { .sb-show-on-checked }
        # Type: dict
        watchtower2_docker_envs_custom: {}
        ```

    ??? variable string "`watchtower_role_docker_env_file`{ .sb-show-on-unchecked }`watchtower2_docker_env_file`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_docker_env_file: "{{ lookup('role_var', '_paths_location', role='watchtower') + '/watchtower.env' if watchtower_env.stat.exists else omit }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_docker_env_file: "{{ lookup('role_var', '_paths_location', role='watchtower') + '/watchtower.env' if watchtower_env.stat.exists else omit }}"
        ```

    <h5>Volumes</h5>

    ??? variable list "`watchtower_role_docker_volumes_default`{ .sb-show-on-unchecked }`watchtower2_docker_volumes_default`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: list
        watchtower_role_docker_volumes_default: 
          - "/var/run/docker.sock:/var/run/docker.sock"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: list
        watchtower2_docker_volumes_default: 
          - "/var/run/docker.sock:/var/run/docker.sock"
        ```

    ??? variable list "`watchtower_role_docker_volumes_custom`{ .sb-show-on-unchecked }`watchtower2_docker_volumes_custom`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: list
        watchtower_role_docker_volumes_custom: []
        ```

        ```yaml { .sb-show-on-checked }
        # Type: list
        watchtower2_docker_volumes_custom: []
        ```

    <h5>Hostname</h5>

    ??? variable string "`watchtower_role_docker_hostname`{ .sb-show-on-unchecked }`watchtower2_docker_hostname`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_docker_hostname: "{{ watchtower_name }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_docker_hostname: "{{ watchtower_name }}"
        ```

    <h5>Networks</h5>

    ??? variable string "`watchtower_role_docker_networks_alias`{ .sb-show-on-unchecked }`watchtower2_docker_networks_alias`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_docker_networks_alias: "{{ watchtower_name }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_docker_networks_alias: "{{ watchtower_name }}"
        ```

    ??? variable list "`watchtower_role_docker_networks_default`{ .sb-show-on-unchecked }`watchtower2_docker_networks_default`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: list
        watchtower_role_docker_networks_default: []
        ```

        ```yaml { .sb-show-on-checked }
        # Type: list
        watchtower2_docker_networks_default: []
        ```

    ??? variable list "`watchtower_role_docker_networks_custom`{ .sb-show-on-unchecked }`watchtower2_docker_networks_custom`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: list
        watchtower_role_docker_networks_custom: []
        ```

        ```yaml { .sb-show-on-checked }
        # Type: list
        watchtower2_docker_networks_custom: []
        ```

    <h5>Restart Policy</h5>

    ??? variable string "`watchtower_role_docker_restart_policy`{ .sb-show-on-unchecked }`watchtower2_docker_restart_policy`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_docker_restart_policy: unless-stopped
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_docker_restart_policy: unless-stopped
        ```

    <h5>State</h5>

    ??? variable string "`watchtower_role_docker_state`{ .sb-show-on-unchecked }`watchtower2_docker_state`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        watchtower_role_docker_state: started
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        watchtower2_docker_state: started
        ```

=== "Global Override Options"

    ??? variable bool "`watchtower_role_autoheal_enabled`{ .sb-show-on-unchecked }`watchtower2_autoheal_enabled`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Enable or disable Autoheal monitoring for containers created when deploying
        # Type: bool (true/false)
        watchtower_role_autoheal_enabled: true
        ```

        ```yaml { .sb-show-on-checked }
        # Enable or disable Autoheal monitoring for containers created when deploying
        # Type: bool (true/false)
        watchtower2_autoheal_enabled: true
        ```

    ??? variable string "`watchtower_role_depends_on`{ .sb-show-on-unchecked }`watchtower2_depends_on`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # List of container dependencies that must be running before containers start
        # Type: string
        watchtower_role_depends_on: ""
        ```

        ```yaml { .sb-show-on-checked }
        # List of container dependencies that must be running before containers start
        # Type: string
        watchtower2_depends_on: ""
        ```

    ??? variable string "`watchtower_role_depends_on_delay`{ .sb-show-on-unchecked }`watchtower2_depends_on_delay`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Delay in seconds before starting containers after dependencies are ready
        # Type: string (quoted number)
        watchtower_role_depends_on_delay: "0"
        ```

        ```yaml { .sb-show-on-checked }
        # Delay in seconds before starting containers after dependencies are ready
        # Type: string (quoted number)
        watchtower2_depends_on_delay: "0"
        ```

    ??? variable string "`watchtower_role_depends_on_healthchecks`{ .sb-show-on-unchecked }`watchtower2_depends_on_healthchecks`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Enable healthcheck waiting for container dependencies
        # Type: string ("true"/"false")
        watchtower_role_depends_on_healthchecks:
        ```

        ```yaml { .sb-show-on-checked }
        # Enable healthcheck waiting for container dependencies
        # Type: string ("true"/"false")
        watchtower2_depends_on_healthchecks:
        ```

    ??? variable bool "`watchtower_role_diun_enabled`{ .sb-show-on-unchecked }`watchtower2_diun_enabled`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Enable or disable Diun update notifications for containers created when deploying
        # Type: bool (true/false)
        watchtower_role_diun_enabled: true
        ```

        ```yaml { .sb-show-on-checked }
        # Enable or disable Diun update notifications for containers created when deploying
        # Type: bool (true/false)
        watchtower2_diun_enabled: true
        ```

    ??? variable bool "`watchtower_role_dns_enabled`{ .sb-show-on-unchecked }`watchtower2_dns_enabled`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Enable or disable automatic DNS record creation for containers
        # Type: bool (true/false)
        watchtower_role_dns_enabled: true
        ```

        ```yaml { .sb-show-on-checked }
        # Enable or disable automatic DNS record creation for containers
        # Type: bool (true/false)
        watchtower2_dns_enabled: true
        ```

    ??? variable bool "`watchtower_role_docker_controller`{ .sb-show-on-unchecked }`watchtower2_docker_controller`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Enable or disable Saltbox Docker Controller management for containers
        # Type: bool (true/false)
        watchtower_role_docker_controller: true
        ```

        ```yaml { .sb-show-on-checked }
        # Enable or disable Saltbox Docker Controller management for containers
        # Type: bool (true/false)
        watchtower2_docker_controller: true
        ```

    ??? variable bool "`watchtower_role_traefik_autodetect_enabled`{ .sb-show-on-unchecked }`watchtower2_traefik_autodetect_enabled`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Enable Traefik autodetect middleware for containers
        # Type: bool (true/false)
        watchtower_role_traefik_autodetect_enabled: false
        ```

        ```yaml { .sb-show-on-checked }
        # Enable Traefik autodetect middleware for containers
        # Type: bool (true/false)
        watchtower2_traefik_autodetect_enabled: false
        ```

    ??? variable bool "`watchtower_role_traefik_crowdsec_enabled`{ .sb-show-on-unchecked }`watchtower2_traefik_crowdsec_enabled`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Enable CrowdSec middleware for containers
        # Type: bool (true/false)
        watchtower_role_traefik_crowdsec_enabled: false
        ```

        ```yaml { .sb-show-on-checked }
        # Enable CrowdSec middleware for containers
        # Type: bool (true/false)
        watchtower2_traefik_crowdsec_enabled: false
        ```

    ??? variable bool "`watchtower_role_traefik_error_pages_enabled`{ .sb-show-on-unchecked }`watchtower2_traefik_error_pages_enabled`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Enable custom error pages middleware for containers
        # Type: bool (true/false)
        watchtower_role_traefik_error_pages_enabled: false
        ```

        ```yaml { .sb-show-on-checked }
        # Enable custom error pages middleware for containers
        # Type: bool (true/false)
        watchtower2_traefik_error_pages_enabled: false
        ```

    ??? variable bool "`watchtower_role_traefik_gzip_enabled`{ .sb-show-on-unchecked }`watchtower2_traefik_gzip_enabled`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Enable gzip compression middleware for containers
        # Type: bool (true/false)
        watchtower_role_traefik_gzip_enabled: false
        ```

        ```yaml { .sb-show-on-checked }
        # Enable gzip compression middleware for containers
        # Type: bool (true/false)
        watchtower2_traefik_gzip_enabled: false
        ```

    ??? variable bool "`watchtower_role_traefik_middleware_http_api_insecure`{ .sb-show-on-unchecked }`watchtower2_traefik_middleware_http_api_insecure`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: bool (true/false)
        watchtower_role_traefik_middleware_http_api_insecure:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: bool (true/false)
        watchtower2_traefik_middleware_http_api_insecure:
        ```

    ??? variable bool "`watchtower_role_traefik_middleware_http_insecure`{ .sb-show-on-unchecked }`watchtower2_traefik_middleware_http_insecure`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: bool (true/false)
        watchtower_role_traefik_middleware_http_insecure:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: bool (true/false)
        watchtower2_traefik_middleware_http_insecure:
        ```

    ??? variable bool "`watchtower_role_traefik_robot_enabled`{ .sb-show-on-unchecked }`watchtower2_traefik_robot_enabled`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Enable robots.txt middleware for containers
        # Type: bool (true/false)
        watchtower_role_traefik_robot_enabled: true
        ```

        ```yaml { .sb-show-on-checked }
        # Enable robots.txt middleware for containers
        # Type: bool (true/false)
        watchtower2_traefik_robot_enabled: true
        ```

    ??? variable bool "`watchtower_role_traefik_tailscale_enabled`{ .sb-show-on-unchecked }`watchtower2_traefik_tailscale_enabled`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Enable Tailscale-specific Traefik configuration for containers
        # Type: bool (true/false)
        watchtower_role_traefik_tailscale_enabled: false
        ```

        ```yaml { .sb-show-on-checked }
        # Enable Tailscale-specific Traefik configuration for containers
        # Type: bool (true/false)
        watchtower2_traefik_tailscale_enabled: false
        ```

    ??? variable bool "`watchtower_role_traefik_wildcard_enabled`{ .sb-show-on-unchecked }`watchtower2_traefik_wildcard_enabled`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Enable wildcard certificate for containers
        # Type: bool (true/false)
        watchtower_role_traefik_wildcard_enabled: true
        ```

        ```yaml { .sb-show-on-checked }
        # Enable wildcard certificate for containers
        # Type: bool (true/false)
        watchtower2_traefik_wildcard_enabled: true
        ```

    ??? variable list "`watchtower_role_web_fqdn_override`{ .sb-show-on-unchecked }`watchtower2_web_fqdn_override`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Override the Traefik fully qualified domain name (FQDN) for containers
        # Type: list
        watchtower_role_web_fqdn_override:
        ```

        ```yaml { .sb-show-on-checked }
        # Override the Traefik fully qualified domain name (FQDN) for containers
        # Type: list
        watchtower2_web_fqdn_override:
        ```

        !!! example sb-show-on-unchecked "Example Override"

            ```yaml
            watchtower_role_web_fqdn_override:
              - "{{ traefik_host }}"
              - "watchtower2.{{ user.domain }}"
              - "watchtower.otherdomain.tld"
            ```

            Note: Include `{{ traefik_host }}` to preserve the default FQDN alongside your custom entries

        !!! example sb-show-on-checked "Example Override"

            ```yaml
            watchtower2_web_fqdn_override:
              - "{{ traefik_host }}"
              - "watchtower2.{{ user.domain }}"
              - "watchtower.otherdomain.tld"
            ```

            Note: Include `{{ traefik_host }}` to preserve the default FQDN alongside your custom entries

    ??? variable string "`watchtower_role_web_host_override`{ .sb-show-on-unchecked }`watchtower2_web_host_override`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Override the Traefik web host configuration for containers
        # Type: string
        watchtower_role_web_host_override:
        ```

        ```yaml { .sb-show-on-checked }
        # Override the Traefik web host configuration for containers
        # Type: string
        watchtower2_web_host_override:
        ```

        !!! example sb-show-on-unchecked "Example Override"

            ```yaml
            watchtower_role_web_host_override: "Host(`{{ traefik_host }}`) || Host(`{{ 'watchtower2.' + user.domain }}`)"
            ```

            Note: Use `{{ traefik_host }}` to include the default host configuration in your custom rule

        !!! example sb-show-on-checked "Example Override"

            ```yaml
            watchtower2_web_host_override: "Host(`{{ traefik_host }}`) || Host(`{{ 'watchtower2.' + user.domain }}`)"
            ```

            Note: Use `{{ traefik_host }}` to include the default host configuration in your custom rule

    ??? variable string "`watchtower_role_web_scheme`{ .sb-show-on-unchecked }`watchtower2_web_scheme`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # URL scheme to use for web access to containers
        # Type: string ("http"/"https")
        watchtower_role_web_scheme:
        ```

        ```yaml { .sb-show-on-checked }
        # URL scheme to use for web access to containers
        # Type: string ("http"/"https")
        watchtower2_web_scheme:
        ```

<!-- END SALTBOX MANAGED VARIABLES SECTION -->