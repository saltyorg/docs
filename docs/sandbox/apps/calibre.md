...
=== "Docker+"
    ...
    ??? variable string "`calibre_role_docker_nvidia`{ .sb-show-on-unchecked }`calibre2_docker_nvidia`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        calibre_role_docker_nvidia:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        calibre2_docker_nvidia:
        ```
