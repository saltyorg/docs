...
=== "Global Override Options"
    ...
    ??? variable list "`postgres_role_paths_folders_list_custom`{ .sb-show-on-unchecked }`postgres2_paths_folders_list_custom`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Extra directories to create
        # Type: list
        postgres_role_paths_folders_list_custom:
        ```

        ```yaml { .sb-show-on-checked }
        # Extra directories to create
        # Type: list
        postgres2_paths_folders_list_custom:
        ```

    ??? variable string "`postgres_role_paths_group`{ .sb-show-on-unchecked }`postgres2_paths_group`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Group for directories created by the role
        # Type: string
        postgres_role_paths_group:
        ```

        ```yaml { .sb-show-on-checked }
        # Group for directories created by the role
        # Type: string
        postgres2_paths_group:
        ```

    ??? variable string "`postgres_role_paths_owner`{ .sb-show-on-unchecked }`postgres2_paths_owner`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Owner for directories created by the role
        # Type: string
        postgres_role_paths_owner:
        ```

        ```yaml { .sb-show-on-checked }
        # Owner for directories created by the role
        # Type: string
        postgres2_paths_owner:
        ```

    ??? variable string "`postgres_role_paths_permissions`{ .sb-show-on-unchecked }`postgres2_paths_permissions`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Permissions for directories created by the role
        # Type: string
        postgres_role_paths_permissions:
        ```

        ```yaml { .sb-show-on-checked }
        # Permissions for directories created by the role
        # Type: string
        postgres2_paths_permissions:
        ```

    ??? variable bool "`postgres_role_paths_recursive`{ .sb-show-on-unchecked }`postgres2_paths_recursive`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Apply owner and group recursively without changing child modes
        # Type: bool (true/false)
        postgres_role_paths_recursive:
        ```

        ```yaml { .sb-show-on-checked }
        # Apply owner and group recursively without changing child modes
        # Type: bool (true/false)
        postgres2_paths_recursive:
        ```
