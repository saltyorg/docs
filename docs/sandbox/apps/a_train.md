...
saltbox_automation:
  ...
  project_description:
    name: A-Train
    summary: |-
      the official Autoscan trigger that listens for changes within Google Drive. It is the successor of Autoscan's Bernard trigger, which unfortunately contains enough logic errors to prompt a rewrite.
    link: https://github.com/m-rots/a-train
---
...
=== "Docker+"
    ...
    ??? variable string "`a_train_role_docker_stop_signal`"

        ```yaml
        # Type: string
        a_train_role_docker_stop_signal:
        ```
    ...
    ??? variable string "`a_train_role_docker_dev_dri`"

        ```yaml
        # Type: string
        a_train_role_docker_dev_dri:
        ```
