    ...
    ??? variable dict "`linkwarden_role_postgres_docker_healthcheck`"
        ...
        linkwarden_role_postgres_docker_healthcheck:
          test:
            ...
            - "pg_isready"
            - "-d"
            - "{{ lookup('role_var', '_postgres_docker_env_db', role='linkwarden') }}"
            - "-U"
            - "{{ lookup('role_var', '_postgres_user', role='linkwarden') }}"
          start_period: 20s
          interval: 30s
          retries: 5
          timeout: 5s
...
=== "Web"
    ...
    ??? variable string "`linkwarden_role_web_url`"
        ...
        ```yaml
        # Type: string
        linkwarden_role_web_url: "{{ lookup('role_web', role='linkwarden', scheme='https') }}"
        ```
...
=== "Traefik"
    ...
    ??? variable string "`linkwarden_role_traefik_middleware_default_api`"

        ```yaml
        # Type: string
        linkwarden_role_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

    ??? variable string "`linkwarden_role_traefik_middleware_custom_api`"

        ```yaml
        # Type: string
        linkwarden_role_traefik_middleware_custom_api: ""
        ```
...
=== "Docker+"
    ...
    ??? variable bool "`linkwarden_role_docker_gpu_enabled`"
        ...
        # Type: bool (true/false)
        linkwarden_role_docker_gpu_enabled: false
        ```

    ??? variable bool "`linkwarden_role_docker_nvidia_disabled`"
        ...
        # Type: bool (true/false)
        linkwarden_role_docker_nvidia_disabled: false
        ```

    ??? variable bool "`linkwarden_role_docker_dev_dri_disabled`"
        ...
        # Type: bool (true/false)
        linkwarden_role_docker_dev_dri_disabled: false
        ```
...
=== "Global Override Options"
    ...
    ??? variable list "`linkwarden_role_docker_networks_alias_custom`"

        ```yaml
        # Type: list
        linkwarden_role_docker_networks_alias_custom:
        ```
    ...
    ??? variable list "`linkwarden_role_paths_folders_list_custom`"

        ```yaml
        # Extra directories to create
        # Type: list
        linkwarden_role_paths_folders_list_custom:
        ```

    ??? variable string "`linkwarden_role_paths_group`"

        ```yaml
        # Group for directories created by the role
        # Type: string
        linkwarden_role_paths_group:
        ```

    ??? variable string "`linkwarden_role_paths_owner`"

        ```yaml
        # Owner for directories created by the role
        # Type: string
        linkwarden_role_paths_owner:
        ```

    ??? variable string "`linkwarden_role_paths_permissions`"

        ```yaml
        # Permissions for directories created by the role
        # Type: string
        linkwarden_role_paths_permissions:
        ```

    ??? variable bool "`linkwarden_role_paths_recursive`"

        ```yaml
        # Apply ownership and permissions recursively
        # Type: bool (true/false)
        linkwarden_role_paths_recursive:
        ```
