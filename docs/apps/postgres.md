...
!!! info
    ...
    ```yaml { .sb-show-on-unchecked title="Applies to all instances of postgres:" }
    postgres_role_web_subdomain: "custom"
    ```

    ```yaml { .sb-show-on-checked title="Applies to a specific instance (e.g., `postgres2`):" }
...
=== "Docker+"
    ...
    ??? variable string "`postgres_role_docker_devices_default`{ .sb-show-on-unchecked }`postgres2_docker_devices_default`{ .sb-show-on-checked }"
