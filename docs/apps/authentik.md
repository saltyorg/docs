    ...
    ??? variable dict "`authentik_role_postgres_docker_healthcheck`"
        ...
        authentik_role_postgres_docker_healthcheck:
          test:
            ...
            - "pg_isready"
            - "-d"
            - "{{ lookup('role_var', '_postgres_docker_env_db', role='authentik') }}"
            - "-U"
            - "{{ lookup('role_var', '_postgres_user', role='authentik', default=lookup('role_var', '_docker_env_user', role='postgres'), default_if_empty=true) }}"
          start_period: 20s
          interval: 30s
          retries: 5
          timeout: 5s
...
=== "Web"
    ...
    ??? variable string "`authentik_role_web_url`"
        ...
        ```yaml
        # Type: string
        authentik_role_web_url: "{{ lookup('role_web', role='authentik', scheme='https') }}"
        ```
    ...
    ??? variable string "`authentik_role_web_host`"
        ...
        ```yaml
        # Type: string
        authentik_role_web_host: "{{ lookup('role_web', role='authentik') }}"
        ```
...
=== "Traefik"
    ...
    ??? variable string "`authentik_role_traefik_middleware_default_api`"

        ```yaml
        # Type: string
        authentik_role_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

    ??? variable string "`authentik_role_traefik_middleware_custom_api`"

        ```yaml
        # Type: string
        authentik_role_traefik_middleware_custom_api: ""
        ```
...
=== "Docker"
    ...
    ??? variable dict "`authentik_role_docker_labels_default`"
        ...
        authentik_role_docker_labels_default:
          ...
          traefik.http.routers.authentik-outpost.tls.certresolver: "{{ traefik_router_certresolver }}"
...
=== "Docker+"
    ...
    ??? variable bool "`authentik_role_docker_gpu_enabled`"
        ...
        # Type: bool (true/false)
        authentik_role_docker_gpu_enabled: false
        ```

    ??? variable bool "`authentik_role_docker_nvidia_disabled`"
        ...
        # Type: bool (true/false)
        authentik_role_docker_nvidia_disabled: false
        ```

    ??? variable bool "`authentik_role_docker_dev_dri_disabled`"
        ...
        # Type: bool (true/false)
        authentik_role_docker_dev_dri_disabled: false
        ```
...
=== "Global Override Options"
    ...
    ??? variable list "`authentik_role_paths_folders_list_custom`"

        ```yaml
        # Extra directories to create
        # Type: list
        authentik_role_paths_folders_list_custom:
        ```

    ??? variable string "`authentik_role_paths_group`"

        ```yaml
        # Group for directories created by the role
        # Type: string
        authentik_role_paths_group:
        ```

    ??? variable string "`authentik_role_paths_owner`"

        ```yaml
        # Owner for directories created by the role
        # Type: string
        authentik_role_paths_owner:
        ```

    ??? variable string "`authentik_role_paths_permissions`"

        ```yaml
        # Permissions for directories created by the role
        # Type: string
        authentik_role_paths_permissions:
        ```

    ??? variable bool "`authentik_role_paths_recursive`"

        ```yaml
        # Apply owner and group recursively without changing child modes
        # Type: bool (true/false)
        authentik_role_paths_recursive:
        ```
