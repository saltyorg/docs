...
1.  **This role supports multiple instances via `overseerr_instances`.**
    ...
    !!! example "Example override"
        ...
        === "Role-scoped"
            ...
            :material-arrow-right-bottom-bold: Applies to all instances of overseerr
        ...
        === "Instance-scoped"
            ...
            :material-arrow-right-bottom-bold: Applies to the instance named overseerr2
...
=== "Web"
    ...
    ??? variable string "`overseerr_role_web_url`{ .sb-show-on-unchecked }`overseerr2_web_url`{ .sb-show-on-checked }"
        ...
        ```yaml { .sb-show-on-unchecked }
        # Type: string
        overseerr_role_web_url: "{{ lookup('role_web', role='overseerr', scheme='https') }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        overseerr2_web_url: "{{ lookup('role_web', role='overseerr', scheme='https') }}"
        ```
...
=== "Traefik"
    ...
    ??? variable string "`overseerr_role_traefik_middleware_default_api`{ .sb-show-on-unchecked }`overseerr2_traefik_middleware_default_api`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        overseerr_role_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        overseerr2_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

    ??? variable string "`overseerr_role_traefik_middleware_custom_api`{ .sb-show-on-unchecked }`overseerr2_traefik_middleware_custom_api`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        overseerr_role_traefik_middleware_custom_api: ""
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        overseerr2_traefik_middleware_custom_api: ""
        ```
...
=== "Docker+"
    ...
    ??? variable bool "`overseerr_role_docker_gpu_enabled`{ .sb-show-on-unchecked }`overseerr2_docker_gpu_enabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        overseerr_role_docker_gpu_enabled: false
        ```
        ...
        # This setting does not install or enable GPU support on the server.
        # Type: bool (true/false)
        overseerr2_docker_gpu_enabled: false
        ```

    ??? variable bool "`overseerr_role_docker_nvidia_disabled`{ .sb-show-on-unchecked }`overseerr2_docker_nvidia_disabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        overseerr_role_docker_nvidia_disabled: false
        ```
        ...
        # Automatic /dev/dri access may remain.
        # Type: bool (true/false)
        overseerr2_docker_nvidia_disabled: false
        ```

    ??? variable bool "`overseerr_role_docker_dev_dri_disabled`{ .sb-show-on-unchecked }`overseerr2_docker_dev_dri_disabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        overseerr_role_docker_dev_dri_disabled: false
        ```
        ...
        # NVIDIA-specific access may remain.
        # Type: bool (true/false)
        overseerr2_docker_dev_dri_disabled: false
        ```
    ...
    ??? variable bool "`overseerr_role_docker_hosts_use_common`{ .sb-show-on-unchecked }`overseerr2_docker_hosts_use_common`{ .sb-show-on-checked }"
    ...
    ??? variable bool "`overseerr_role_docker_volumes_global`{ .sb-show-on-unchecked }`overseerr2_docker_volumes_global`{ .sb-show-on-checked }"
    ...
    ??? variable int "`overseerr_role_docker_create_timeout`{ .sb-show-on-unchecked }`overseerr2_docker_create_timeout`{ .sb-show-on-checked }"
    ...
    ??? variable bool "`overseerr_role_docker_labels_use_common`{ .sb-show-on-unchecked }`overseerr2_docker_labels_use_common`{ .sb-show-on-checked }"
...
=== "Global Override Options"
    ...
    ??? variable list "`overseerr_role_paths_folders_list_custom`{ .sb-show-on-unchecked }`overseerr2_paths_folders_list_custom`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Extra directories to create
        # Type: list
        overseerr_role_paths_folders_list_custom:
        ```

        ```yaml { .sb-show-on-checked }
        # Extra directories to create
        # Type: list
        overseerr2_paths_folders_list_custom:
        ```

    ??? variable string "`overseerr_role_paths_group`{ .sb-show-on-unchecked }`overseerr2_paths_group`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Group for directories created by the role
        # Type: string
        overseerr_role_paths_group:
        ```

        ```yaml { .sb-show-on-checked }
        # Group for directories created by the role
        # Type: string
        overseerr2_paths_group:
        ```

    ??? variable string "`overseerr_role_paths_owner`{ .sb-show-on-unchecked }`overseerr2_paths_owner`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Owner for directories created by the role
        # Type: string
        overseerr_role_paths_owner:
        ```

        ```yaml { .sb-show-on-checked }
        # Owner for directories created by the role
        # Type: string
        overseerr2_paths_owner:
        ```

    ??? variable string "`overseerr_role_paths_permissions`{ .sb-show-on-unchecked }`overseerr2_paths_permissions`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Permissions for directories created by the role
        # Type: string
        overseerr_role_paths_permissions:
        ```

        ```yaml { .sb-show-on-checked }
        # Permissions for directories created by the role
        # Type: string
        overseerr2_paths_permissions:
        ```

    ??? variable bool "`overseerr_role_paths_recursive`{ .sb-show-on-unchecked }`overseerr2_paths_recursive`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Apply owner and group recursively without changing child modes
        # Type: bool (true/false)
        overseerr_role_paths_recursive:
        ```

        ```yaml { .sb-show-on-checked }
        # Apply owner and group recursively without changing child modes
        # Type: bool (true/false)
        overseerr2_paths_recursive:
        ```

    ??? variable string "`overseerr_role_traefik_api_middleware_http`{ .sb-show-on-unchecked }`overseerr2_traefik_api_middleware_http`{ .sb-show-on-checked }"
    ...
    ??? variable string "`overseerr_role_traefik_middleware_http`{ .sb-show-on-unchecked }`overseerr2_traefik_middleware_http`{ .sb-show-on-checked }"
    ...
    ??? variable string "`overseerr_role_traefik_priority`{ .sb-show-on-unchecked }`overseerr2_traefik_priority`{ .sb-show-on-checked }"
    ...
    ??? variable string "`overseerr_role_web_api_http_port`{ .sb-show-on-unchecked }`overseerr2_web_api_http_port`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string (quoted number)
        overseerr_role_web_api_http_port:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string (quoted number)
        overseerr2_web_api_http_port:
        ```

    ??? variable string "`overseerr_role_web_api_http_scheme`{ .sb-show-on-unchecked }`overseerr2_web_api_http_scheme`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string ("http"/"https")
        overseerr_role_web_api_http_scheme:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string ("http"/"https")
        overseerr2_web_api_http_scheme:
        ```

    ??? variable dict "`overseerr_role_web_api_http_serverstransport`{ .sb-show-on-unchecked }`overseerr2_web_api_http_serverstransport`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: dict/omit
        overseerr_role_web_api_http_serverstransport:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: dict/omit
        overseerr2_web_api_http_serverstransport:
        ```

    ??? variable string "`overseerr_role_web_api_port`{ .sb-show-on-unchecked }`overseerr2_web_api_port`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string (quoted number)
        overseerr_role_web_api_port:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string (quoted number)
        overseerr2_web_api_port:
        ```

    ??? variable string "`overseerr_role_web_api_scheme`{ .sb-show-on-unchecked }`overseerr2_web_api_scheme`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string ("http"/"https")
        overseerr_role_web_api_scheme:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string ("http"/"https")
        overseerr2_web_api_scheme:
        ```

    ??? variable dict "`overseerr_role_web_api_serverstransport`{ .sb-show-on-unchecked }`overseerr2_web_api_serverstransport`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: dict/omit
        overseerr_role_web_api_serverstransport:
        ```
    ...
    ??? variable string "`overseerr_role_web_http_port`{ .sb-show-on-unchecked }`overseerr2_web_http_port`{ .sb-show-on-checked }"
    ...
    ??? variable string "`overseerr_role_web_http_scheme`{ .sb-show-on-unchecked }`overseerr2_web_http_scheme`{ .sb-show-on-checked }"
