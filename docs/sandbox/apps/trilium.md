...
saltbox_automation:
  app_links:
    - name: Manual
      url: https://github.com/zadam/trilium/wiki
      type: documentation
      purpose: manual
    - name: Releases
      url: https://hub.docker.com/r/zadam/trilium/tags
      type: docker
      purpose: release
    - name: Community
      url:
...
[:fontawesome-solid-book-open:**Manual**](https://github.com/zadam/trilium/wiki){ .md-button .md-button--stretch }
...
=== "Web"
    ...
    ??? variable string "`trilium_role_web_url`"
        ...
        ```yaml
        # Type: string
        trilium_role_web_url: "{{ lookup('role_web', role='trilium', scheme='https') }}"
        ```
...
=== "Traefik"
    ...
    ??? variable string "`trilium_role_traefik_middleware_default_api`"

        ```yaml
        # Type: string
        trilium_role_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

    ??? variable string "`trilium_role_traefik_middleware_custom_api`"

        ```yaml
        # Type: string
        trilium_role_traefik_middleware_custom_api: ""
        ```
...
=== "Docker"
    ...
    ??? variable list "`trilium_role_docker_volumes_default`"
        ...
        trilium_role_docker_volumes_default:
          - "{{ lookup('role_var', '_paths_location', role='trilium') }}:/home/node/trilium-data"
        ```
...
=== "Docker+"
    ...
    ??? variable bool "`trilium_role_docker_gpu_enabled`"
        ...
        # Type: bool (true/false)
        trilium_role_docker_gpu_enabled: false
        ```

    ??? variable bool "`trilium_role_docker_nvidia_disabled`"
        ...
        # Type: bool (true/false)
        trilium_role_docker_nvidia_disabled: false
        ```

    ??? variable bool "`trilium_role_docker_dev_dri_disabled`"
        ...
        # Type: bool (true/false)
        trilium_role_docker_dev_dri_disabled: false
        ```
    ...
    ??? variable bool "`trilium_role_docker_force_kill`"
    ...
    ??? variable string "`trilium_role_docker_stop_signal`"

        ```yaml
        # Type: string
        trilium_role_docker_stop_signal:
        ```
    ...
    ??? variable list "`trilium_role_docker_entrypoint`"
    ...
    ??? variable dict "`trilium_role_docker_sysctls`"
...
=== "Global Override Options"
    ...
    ??? variable list "`trilium_role_docker_networks_alias_custom`"

        ```yaml
        # Type: list
        trilium_role_docker_networks_alias_custom:
        ```
    ...
    ??? variable list "`trilium_role_paths_folders_list_custom`"

        ```yaml
        # Extra directories to create
        # Type: list
        trilium_role_paths_folders_list_custom:
        ```

    ??? variable string "`trilium_role_paths_group`"

        ```yaml
        # Group for directories created by the role
        # Type: string
        trilium_role_paths_group:
        ```

    ??? variable string "`trilium_role_paths_owner`"

        ```yaml
        # Owner for directories created by the role
        # Type: string
        trilium_role_paths_owner:
        ```

    ??? variable string "`trilium_role_paths_permissions`"

        ```yaml
        # Permissions for directories created by the role
        # Type: string
        trilium_role_paths_permissions:
        ```

    ??? variable bool "`trilium_role_paths_recursive`"

        ```yaml
        # Apply ownership and permissions recursively
        # Type: bool (true/false)
        trilium_role_paths_recursive:
        ```

    ??? variable list "`trilium_role_themepark_addons`"
