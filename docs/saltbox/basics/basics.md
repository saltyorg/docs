...
<sup><b name="f5">[5](#a5)</b> There is presently a 750GB/day upload limitation on Google accounts.  The standard Saltbox setup will describe setting up a Google Drive remote pointed at your My Drive.  This limit can be eliminated by cycling through a set of service accounts [each of which can upload 750GB] to upload to one or more Shared Drives [aka Teamdrives].  TODO ADD THIS DOCUMENT HERE This setup is not discussed in this wiki as yet; there is a document linked in the Discord [`chazlarson-tip44` channel] that discusses it.  </sup>