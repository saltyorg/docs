...
saltbox_automation:
  app_links:
    - name: Manual
      ...
      purpose: manual
    - name: Releases
      url: https://hub.docker.com/r/linuxserver/nginx/tags
      type: docker
      purpose: release
    - name: Community
...
[NGINX](https://nginx.org) is a high-performance web server, reverse proxy, and load balancer. This role deploys Nginx using the LinuxServer.io container, providing a simple way to host static websites or act as a reverse proxy for your applications.

<div class="grid grid--buttons" markdown data-search-exclude>

[:fontawesome-solid-book-open:**Manual**](https://nginx.org/en/docs){ .md-button .md-button--stretch }
...
Nginx is deployed with configuration files at `/opt/nginx/`. Place website files in `/opt/nginx/www/` and edit site configs in `/opt/nginx/nginx/site-confs/`. Restart with `docker restart nginx` to apply changes.
...
1.  **This role supports multiple instances via `nginx_instances`.**
    ...
    !!! example "Example override"
        ...
        === "Role-scoped"
            ...
            :material-arrow-right-bottom-bold: Applies to all instances of nginx
        ...
        === "Instance-scoped"
            ...
            :material-arrow-right-bottom-bold: Applies to the instance named nginx2
...
=== "Traefik"
    ...
    ??? variable string "`nginx_role_traefik_middleware_default_api`{ .sb-show-on-unchecked }`nginx2_traefik_middleware_default_api`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        nginx_role_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        nginx2_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

    ??? variable string "`nginx_role_traefik_middleware_custom_api`{ .sb-show-on-unchecked }`nginx2_traefik_middleware_custom_api`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        nginx_role_traefik_middleware_custom_api: ""
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        nginx2_traefik_middleware_custom_api: ""
        ```
...
=== "Docker"
    ...
    ??? variable list "`nginx_role_docker_volumes_default`{ .sb-show-on-unchecked }`nginx2_docker_volumes_default`{ .sb-show-on-checked }"
        ...
        nginx_role_docker_volumes_default:
          - "{{ lookup('role_var', '_paths_location', role='nginx') }}:/config"
        ```
...
=== "Docker+"
    ...
    ??? variable bool "`nginx_role_docker_gpu_enabled`{ .sb-show-on-unchecked }`nginx2_docker_gpu_enabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        nginx_role_docker_gpu_enabled: false
        ```
        ...
        # This setting does not install or enable GPU support on the server.
        # Type: bool (true/false)
        nginx2_docker_gpu_enabled: false
        ```

    ??? variable bool "`nginx_role_docker_nvidia_disabled`{ .sb-show-on-unchecked }`nginx2_docker_nvidia_disabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        nginx_role_docker_nvidia_disabled: false
        ```
        ...
        # Automatic /dev/dri access may remain.
        # Type: bool (true/false)
        nginx2_docker_nvidia_disabled: false
        ```

    ??? variable bool "`nginx_role_docker_dev_dri_disabled`{ .sb-show-on-unchecked }`nginx2_docker_dev_dri_disabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        nginx_role_docker_dev_dri_disabled: false
        ```
        ...
        # NVIDIA-specific access may remain.
        # Type: bool (true/false)
        nginx2_docker_dev_dri_disabled: false
        ```
    ...
    ??? variable bool "`nginx_role_docker_force_kill`{ .sb-show-on-unchecked }`nginx2_docker_force_kill`{ .sb-show-on-checked }"
    ...
    ??? variable string "`nginx_role_docker_stop_signal`{ .sb-show-on-unchecked }`nginx2_docker_stop_signal`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        nginx_role_docker_stop_signal:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        nginx2_docker_stop_signal:
        ```
    ...
    ??? variable list "`nginx_role_docker_entrypoint`{ .sb-show-on-unchecked }`nginx2_docker_entrypoint`{ .sb-show-on-checked }"
    ...
    ??? variable dict "`nginx_role_docker_sysctls`{ .sb-show-on-unchecked }`nginx2_docker_sysctls`{ .sb-show-on-checked }"
...
=== "Global Override Options"
    ...
    ??? variable list "`nginx_role_docker_networks_alias_custom`{ .sb-show-on-unchecked }`nginx2_docker_networks_alias_custom`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: list
        nginx_role_docker_networks_alias_custom:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: list
        nginx2_docker_networks_alias_custom:
        ```
    ...
    ??? variable list "`nginx_role_paths_folders_list_custom`{ .sb-show-on-unchecked }`nginx2_paths_folders_list_custom`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Extra directories to create
        # Type: list
        nginx_role_paths_folders_list_custom:
        ```

        ```yaml { .sb-show-on-checked }
        # Extra directories to create
        # Type: list
        nginx2_paths_folders_list_custom:
        ```

    ??? variable string "`nginx_role_paths_group`{ .sb-show-on-unchecked }`nginx2_paths_group`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Group for directories created by the role
        # Type: string
        nginx_role_paths_group:
        ```
    ...
    ??? variable string "`nginx_role_paths_owner`{ .sb-show-on-unchecked }`nginx2_paths_owner`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Owner for directories created by the role
        # Type: string
        nginx_role_paths_owner:
        ```
    ...
    ??? variable string "`nginx_role_paths_permissions`{ .sb-show-on-unchecked }`nginx2_paths_permissions`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Permissions for directories created by the role
        # Type: string
        nginx_role_paths_permissions:
        ```

        ```yaml { .sb-show-on-checked }
        # Permissions for directories created by the role
        # Type: string
        nginx2_paths_permissions:
        ```

    ??? variable bool "`nginx_role_paths_recursive`{ .sb-show-on-unchecked }`nginx2_paths_recursive`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Apply owner and group recursively without changing child modes
        # Type: bool (true/false)
        nginx_role_paths_recursive:
        ```

        ```yaml { .sb-show-on-checked }
        # Apply owner and group recursively without changing child modes
        # Type: bool (true/false)
        nginx2_paths_recursive:
        ```

    ??? variable list "`nginx_role_themepark_addons`{ .sb-show-on-unchecked }`nginx2_themepark_addons`{ .sb-show-on-checked }"
    ...
    ??? variable string "`nginx_role_web_api_http_port`{ .sb-show-on-unchecked }`nginx2_web_api_http_port`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string (quoted number)
        nginx_role_web_api_http_port:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string (quoted number)
        nginx2_web_api_http_port:
        ```

    ??? variable string "`nginx_role_web_api_http_scheme`{ .sb-show-on-unchecked }`nginx2_web_api_http_scheme`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string ("http"/"https")
        nginx_role_web_api_http_scheme:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string ("http"/"https")
        nginx2_web_api_http_scheme:
        ```

    ??? variable dict "`nginx_role_web_api_http_serverstransport`{ .sb-show-on-unchecked }`nginx2_web_api_http_serverstransport`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: dict/omit
        nginx_role_web_api_http_serverstransport:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: dict/omit
        nginx2_web_api_http_serverstransport:
        ```

    ??? variable string "`nginx_role_web_api_port`{ .sb-show-on-unchecked }`nginx2_web_api_port`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string (quoted number)
        nginx_role_web_api_port:
        ```
    ...
    ??? variable string "`nginx_role_web_api_scheme`{ .sb-show-on-unchecked }`nginx2_web_api_scheme`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string ("http"/"https")
        nginx_role_web_api_scheme:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string ("http"/"https")
        nginx2_web_api_scheme:
        ```

    ??? variable dict "`nginx_role_web_api_serverstransport`{ .sb-show-on-unchecked }`nginx2_web_api_serverstransport`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: dict/omit
        nginx_role_web_api_serverstransport:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: dict/omit
        nginx2_web_api_serverstransport:
        ```
