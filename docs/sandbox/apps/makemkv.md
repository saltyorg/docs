...
saltbox_automation:
  app_links:
    - name: Manual
      url: https://www.makemkv.com/onlinehelp
      type: documentation
      purpose: manual
    - name: Releases
      url: https://hub.docker.com/r/jlesage/makemkv/tags
      type: docker
      purpose: release
    - name: Community
      url:
  ...
  project_description:
    name: MakeMKV
    summary: |-
      your one-click solution to convert video that you own into free and patents-unencumbered format that can be played everywhere. MakeMKV is a format converter, otherwise called "transcoder". It converts the video clips from proprietary (and usually encrypted) disc into a set of MKV files, preserving most information but not changing it in any way. The MKV format can store multiple video/audio tracks with all meta-information and preserve chapters.
    link: http://www.makemkv.com/
---
...
[:fontawesome-solid-book-open:**Manual**](https://www.makemkv.com/onlinehelp){ .md-button .md-button--stretch }
...
=== "Web"
    ...
    ??? variable string "`makemkv_role_web_url`"
        ...
        ```yaml
        # Type: string
        makemkv_role_web_url: "{{ lookup('role_web', role='makemkv', scheme='https') }}"
        ```
...
=== "Traefik"
    ...
    ??? variable string "`makemkv_role_traefik_middleware_default_api`"

        ```yaml
        # Type: string
        makemkv_role_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

    ??? variable string "`makemkv_role_traefik_middleware_custom_api`"

        ```yaml
        # Type: string
        makemkv_role_traefik_middleware_custom_api: ""
        ```
...
=== "Docker+"
    ...
    ??? variable bool "`makemkv_role_docker_gpu_enabled`"
        ...
        # Type: bool (true/false)
        makemkv_role_docker_gpu_enabled: false
        ```

    ??? variable bool "`makemkv_role_docker_nvidia_disabled`"
        ...
        # Type: bool (true/false)
        makemkv_role_docker_nvidia_disabled: false
        ```

    ??? variable bool "`makemkv_role_docker_dev_dri_disabled`"
        ...
        # Type: bool (true/false)
        makemkv_role_docker_dev_dri_disabled: false
        ```
    ...
    ??? variable bool "`makemkv_role_docker_force_kill`"
    ...
    ??? variable string "`makemkv_role_docker_stop_signal`"

        ```yaml
        # Type: string
        makemkv_role_docker_stop_signal:
        ```
    ...
    ??? variable list "`makemkv_role_docker_entrypoint`"
    ...
    ??? variable dict "`makemkv_role_docker_sysctls`"
...
=== "Global Override Options"
    ...
    ??? variable list "`makemkv_role_docker_networks_alias_custom`"

        ```yaml
        # Type: list
        makemkv_role_docker_networks_alias_custom:
        ```
    ...
    ??? variable string "`makemkv_role_web_api_http_port`"

        ```yaml
        # Type: string (quoted number)
        makemkv_role_web_api_http_port:
        ```

    ??? variable string "`makemkv_role_web_api_http_scheme`"

        ```yaml
        # Type: string ("http"/"https")
        makemkv_role_web_api_http_scheme:
        ```

    ??? variable dict "`makemkv_role_web_api_http_serverstransport`"

        ```yaml
        # Type: dict/omit
        makemkv_role_web_api_http_serverstransport:
        ```

    ??? variable string "`makemkv_role_web_api_port`"

        ```yaml
        # Type: string (quoted number)
        makemkv_role_web_api_port:
        ```

    ??? variable string "`makemkv_role_web_api_scheme`"

        ```yaml
        # Type: string ("http"/"https")
        makemkv_role_web_api_scheme:
        ```

    ??? variable dict "`makemkv_role_web_api_serverstransport`"

        ```yaml
        # Type: dict/omit
        makemkv_role_web_api_serverstransport:
        ```
