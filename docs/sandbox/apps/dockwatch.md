...
saltbox_automation:
  app_links:
    - name: Manual
      ...
      purpose: manual
    - name: Releases
      url: https://github.com/notifiarr/dockwatch/pkgs/container/dockwatch
      type: releases
      purpose: release
    - name: Community
      url:
...
[Dockwatch](https://github.com/Notifiarr/dockwatch) is a web-based tool developed by Notifiarr that provides a user-friendly interface for managing Docker container updates and notifications.
...
[:fontawesome-solid-newspaper:**Releases**](https://github.com/notifiarr/dockwatch/pkgs/container/dockwatch){ .md-button .md-button--stretch }
...
Dockwatch is likely unable to take action on containers due to the security posture of the Docker socket proxy. You can override this behavior to allow Dockwatch to take actions by adding `dockwatch_post_enable: true` to your inventory file.
...
Visit <https://dockwatch.iYOUR_DOMAIN_NAMEi>.
...
=== "Traefik"
    ...
    ??? variable string "`dockwatch_role_traefik_middleware_default_api`"

        ```yaml
        # Type: string
        dockwatch_role_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

    ??? variable string "`dockwatch_role_traefik_middleware_custom_api`"

        ```yaml
        # Type: string
        dockwatch_role_traefik_middleware_custom_api: ""
        ```
...
=== "Docker+"
    ...
    ??? variable bool "`dockwatch_role_docker_gpu_enabled`"
        ...
        # Type: bool (true/false)
        dockwatch_role_docker_gpu_enabled: false
        ```

    ??? variable bool "`dockwatch_role_docker_nvidia_disabled`"
        ...
        # Type: bool (true/false)
        dockwatch_role_docker_nvidia_disabled: false
        ```

    ??? variable bool "`dockwatch_role_docker_dev_dri_disabled`"
        ...
        # Type: bool (true/false)
        dockwatch_role_docker_dev_dri_disabled: false
        ```

    <h5>Resource Limits</h5>

    ??? variable int "`dockwatch_role_docker_blkio_weight`"

        ```yaml
        # Type: int
        dockwatch_role_docker_blkio_weight:
        ```

    ??? variable int "`dockwatch_role_docker_cpu_period`"

        ```yaml
        # Type: int
        dockwatch_role_docker_cpu_period:
        ```

    ??? variable int "`dockwatch_role_docker_cpu_quota`"

        ```yaml
        # Type: int
        dockwatch_role_docker_cpu_quota:
        ```

    ??? variable int "`dockwatch_role_docker_cpu_shares`"

        ```yaml
        # Type: int
        dockwatch_role_docker_cpu_shares:
        ```
    ...
    ??? variable string "`dockwatch_role_docker_cpuset_cpus`"

        ```yaml
        # Type: string
        dockwatch_role_docker_cpuset_cpus:
        ```

    ??? variable string "`dockwatch_role_docker_cpuset_mems`"

        ```yaml
        # Type: string
        dockwatch_role_docker_cpuset_mems:
        ```

    ??? variable string "`dockwatch_role_docker_kernel_memory`"

        ```yaml
        # Type: string
        dockwatch_role_docker_kernel_memory:
        ```

    ??? variable string "`dockwatch_role_docker_memory`"

        ```yaml
        # Type: string
        dockwatch_role_docker_memory:
        ```

    ??? variable string "`dockwatch_role_docker_memory_reservation`"

        ```yaml
        # Type: string
        dockwatch_role_docker_memory_reservation:
        ```

    ??? variable string "`dockwatch_role_docker_memory_swap`"

        ```yaml
        # Type: string
        dockwatch_role_docker_memory_swap:
        ```

    ??? variable int "`dockwatch_role_docker_memory_swappiness`"

        ```yaml
        # Type: int
        dockwatch_role_docker_memory_swappiness:
        ```

    ??? variable string "`dockwatch_role_docker_shm_size`"

        ```yaml
        # Type: string
        dockwatch_role_docker_shm_size:
        ```

    <h5>Security & Devices</h5>

    ??? variable list "`dockwatch_role_docker_cap_drop`"

        ```yaml
        # Type: list
        dockwatch_role_docker_cap_drop:
        ```

    ??? variable string "`dockwatch_role_docker_cgroupns_mode`"

        ```yaml
        # Type: string
        dockwatch_role_docker_cgroupns_mode:
        ```

    ??? variable list "`dockwatch_role_docker_device_cgroup_rules`"

        ```yaml
        # Type: list
        dockwatch_role_docker_device_cgroup_rules:
        ```

    ??? variable list "`dockwatch_role_docker_device_read_bps`"

        ```yaml
        # Type: list
        dockwatch_role_docker_device_read_bps:
        ```

    ??? variable list "`dockwatch_role_docker_device_read_iops`"

        ```yaml
        # Type: list
        dockwatch_role_docker_device_read_iops:
        ```

    ??? variable list "`dockwatch_role_docker_device_requests`"

        ```yaml
        # Type: list
        dockwatch_role_docker_device_requests:
        ```

    ??? variable list "`dockwatch_role_docker_device_write_bps`"

        ```yaml
        # Type: list
        dockwatch_role_docker_device_write_bps:
        ```

    ??? variable list "`dockwatch_role_docker_device_write_iops`"

        ```yaml
        # Type: list
        dockwatch_role_docker_device_write_iops:
        ```

    ??? variable list "`dockwatch_role_docker_devices`"
    ...
    ??? variable list "`dockwatch_role_docker_groups`"
        ...
        ```yaml
        # Type: list
        dockwatch_role_docker_groups:
        ```

    ??? variable bool "`dockwatch_role_docker_privileged`"

        ```yaml
        # Type: bool (true/false)
        dockwatch_role_docker_privileged:
        ```

    ??? variable list "`dockwatch_role_docker_security_opts`"

        ```yaml
        # Type: list
        dockwatch_role_docker_security_opts:
        ```

    ??? variable string "`dockwatch_role_docker_user`"

        ```yaml
        # Type: string
        dockwatch_role_docker_user:
        ```

    ??? variable string "`dockwatch_role_docker_userns_mode`"

        ```yaml
        # Type: string
        dockwatch_role_docker_userns_mode:
        ```

    <h5>Networking</h5>

    ??? variable list "`dockwatch_role_docker_dns_opts`"

        ```yaml
        # Type: list
        dockwatch_role_docker_dns_opts:
        ```

    ??? variable list "`dockwatch_role_docker_dns_search_domains`"

        ```yaml
        # Type: list
        dockwatch_role_docker_dns_search_domains:
        ```

    ??? variable list "`dockwatch_role_docker_dns_servers`"

        ```yaml
        # Type: list
        dockwatch_role_docker_dns_servers:
        ```

    ??? variable string "`dockwatch_role_docker_domainname`"

        ```yaml
        # Type: string
        dockwatch_role_docker_domainname:
        ```

    ??? variable list "`dockwatch_role_docker_exposed_ports`"

        ```yaml
        # Type: list
        dockwatch_role_docker_exposed_ports:
        ```

    ??? variable dict "`dockwatch_role_docker_hosts`"

        ```yaml
        # Type: dict
        dockwatch_role_docker_hosts:
        ```

    ??? variable bool "`dockwatch_role_docker_hosts_use_common`"

        ```yaml
        # Type: bool (true/false)
        dockwatch_role_docker_hosts_use_common:
        ```

    ??? variable string "`dockwatch_role_docker_ipc_mode`"

        ```yaml
        # Type: string
        dockwatch_role_docker_ipc_mode:
        ```

    ??? variable list "`dockwatch_role_docker_links`"

        ```yaml
        # Type: list
        dockwatch_role_docker_links:
        ```

    ??? variable string "`dockwatch_role_docker_network_mode`"

        ```yaml
        # Type: string
        dockwatch_role_docker_network_mode:
        ```

    ??? variable string "`dockwatch_role_docker_pid_mode`"

        ```yaml
        # Type: string
        dockwatch_role_docker_pid_mode:
        ```

    ??? variable list "`dockwatch_role_docker_ports`"

        ```yaml
        # Type: list
        dockwatch_role_docker_ports:
        ```

    ??? variable string "`dockwatch_role_docker_uts`"

        ```yaml
        # Type: string
        dockwatch_role_docker_uts:
        ```

    <h5>Storage</h5>

    ??? variable bool "`dockwatch_role_docker_keep_volumes`"

        ```yaml
        # Type: bool (true/false)
        dockwatch_role_docker_keep_volumes:
        ```

    ??? variable list "`dockwatch_role_docker_mounts`"

        ```yaml
        # Type: list
        dockwatch_role_docker_mounts:
        ```

    ??? variable dict "`dockwatch_role_docker_storage_opts`"

        ```yaml
        # Type: dict
        dockwatch_role_docker_storage_opts:
        ```

    ??? variable list "`dockwatch_role_docker_tmpfs`"

        ```yaml
        # Type: list
        dockwatch_role_docker_tmpfs:
        ```

    ??? variable string "`dockwatch_role_docker_volume_driver`"

        ```yaml
        # Type: string
        dockwatch_role_docker_volume_driver:
        ```

    ??? variable list "`dockwatch_role_docker_volumes_from`"

        ```yaml
        # Type: list
        dockwatch_role_docker_volumes_from:
        ```

    ??? variable bool "`dockwatch_role_docker_volumes_global`"

        ```yaml
        # Type: bool (true/false)
        dockwatch_role_docker_volumes_global:
        ```

    ??? variable string "`dockwatch_role_docker_working_dir`"

        ```yaml
        # Type: string
        dockwatch_role_docker_working_dir:
        ```

    <h5>Monitoring & Lifecycle</h5>

    ??? variable bool "`dockwatch_role_docker_auto_remove`"

        ```yaml
        # Type: bool (true/false)
        dockwatch_role_docker_auto_remove:
        ```

    ??? variable bool "`dockwatch_role_docker_cleanup`"
    ...
    ??? variable dict "`dockwatch_role_docker_healthcheck`"

        ```yaml
        # Type: dict
        dockwatch_role_docker_healthcheck:
        ```
    ...
    ??? variable bool "`dockwatch_role_docker_init`"

        ```yaml
        # Type: bool (true/false)
        dockwatch_role_docker_init:
        ```

    ??? variable string "`dockwatch_role_docker_kill_signal`"

        ```yaml
        # Type: string
        dockwatch_role_docker_kill_signal:
        ```

    ??? variable string "`dockwatch_role_docker_log_driver`"

        ```yaml
        # Type: string
        dockwatch_role_docker_log_driver:
        ```

    ??? variable dict "`dockwatch_role_docker_log_options`"

        ```yaml
        # Type: dict
        dockwatch_role_docker_log_options:
        ```

    ??? variable bool "`dockwatch_role_docker_oom_killer`"

        ```yaml
        # Type: bool (true/false)
        dockwatch_role_docker_oom_killer:
        ```

    ??? variable int "`dockwatch_role_docker_oom_score_adj`"

        ```yaml
        # Type: int
        dockwatch_role_docker_oom_score_adj:
        ```

    ??? variable bool "`dockwatch_role_docker_output_logs`"

        ```yaml
        # Type: bool (true/false)
        dockwatch_role_docker_output_logs:
        ```

    ??? variable bool "`dockwatch_role_docker_paused`"

        ```yaml
        # Type: bool (true/false)
        dockwatch_role_docker_paused:
        ```

    ??? variable bool "`dockwatch_role_docker_recreate`"

        ```yaml
        # Type: bool (true/false)
        dockwatch_role_docker_recreate:
        ```

    ??? variable int "`dockwatch_role_docker_restart_retries`"
    ...
    ??? variable int "`dockwatch_role_docker_stop_timeout`"
        ...
        ```yaml
        # Type: int
        dockwatch_role_docker_stop_timeout:
        ```

    <h5>Other Options</h5>

    ??? variable list "`dockwatch_role_docker_capabilities`"

        ```yaml
        # Type: list
        dockwatch_role_docker_capabilities:
        ```

    ??? variable string "`dockwatch_role_docker_cgroup_parent`"

        ```yaml
        # Type: string
        dockwatch_role_docker_cgroup_parent:
        ```

    ??? variable list "`dockwatch_role_docker_commands`"

        ```yaml
        # Type: list
        dockwatch_role_docker_commands:
        ```

    ??? variable int "`dockwatch_role_docker_create_timeout`"
    ...
    ??? variable string "`dockwatch_role_docker_env_file`"

        ```yaml
        # Type: string
        dockwatch_role_docker_env_file:
        ```

    ??? variable dict "`dockwatch_role_docker_labels`"

        ```yaml
        # Type: dict
        dockwatch_role_docker_labels:
        ```

    ??? variable bool "`dockwatch_role_docker_labels_use_common`"
    ...
    ??? variable bool "`dockwatch_role_docker_read_only`"
        ...
        ```yaml
        # Type: bool (true/false)
        dockwatch_role_docker_read_only:
        ```

    ??? variable string "`dockwatch_role_docker_runtime`"
    ...
    ??? variable list "`dockwatch_role_docker_ulimits`"

        ```yaml
        # Type: list
        dockwatch_role_docker_ulimits:
        ```
...
=== "Global Override Options"
    ...
    ??? variable list "`dockwatch_role_paths_folders_list_custom`"

        ```yaml
        # Extra directories to create
        # Type: list
        dockwatch_role_paths_folders_list_custom:
        ```

    ??? variable string "`dockwatch_role_paths_group`"

        ```yaml
        # Group for directories created by the role
        # Type: string
        dockwatch_role_paths_group:
        ```

    ??? variable string "`dockwatch_role_paths_owner`"

        ```yaml
        # Owner for directories created by the role
        # Type: string
        dockwatch_role_paths_owner:
        ```

    ??? variable string "`dockwatch_role_paths_permissions`"

        ```yaml
        # Permissions for directories created by the role
        # Type: string
        dockwatch_role_paths_permissions:
        ```

    ??? variable bool "`dockwatch_role_paths_recursive`"

        ```yaml
        # Apply ownership and permissions recursively
        # Type: bool (true/false)
        dockwatch_role_paths_recursive:
        ```
    ...
    ??? variable string "`dockwatch_role_themepark_app`"

        ```yaml
        # Type: string
        dockwatch_role_themepark_app:
        ```

    ??? variable string "`dockwatch_role_themepark_theme`"
    ...
    ??? variable string "`dockwatch_role_traefik_api_middleware_http`"

        ```yaml
        # Type: string
        dockwatch_role_traefik_api_middleware_http:
        ```
    ...
    ??? variable string "`dockwatch_role_traefik_middleware_http`"

        ```yaml
        # Type: string
        dockwatch_role_traefik_middleware_http:
        ```
    ...
    ??? variable string "`dockwatch_role_traefik_priority`"

        ```yaml
        # Type: string
        dockwatch_role_traefik_priority:
        ```
    ...
    ??? variable string "`dockwatch_role_web_api_http_port`"

        ```yaml
        # Type: string (quoted number)
        dockwatch_role_web_api_http_port:
        ```

    ??? variable string "`dockwatch_role_web_api_http_scheme`"

        ```yaml
        # Type: string ("http"/"https")
        dockwatch_role_web_api_http_scheme:
        ```

    ??? variable dict "`dockwatch_role_web_api_http_serverstransport`"

        ```yaml
        # Type: dict/omit
        dockwatch_role_web_api_http_serverstransport:
        ```

    ??? variable string "`dockwatch_role_web_api_port`"

        ```yaml
        # Type: string (quoted number)
        dockwatch_role_web_api_port:
        ```

    ??? variable string "`dockwatch_role_web_api_scheme`"

        ```yaml
        # Type: string ("http"/"https")
        dockwatch_role_web_api_scheme:
        ```

    ??? variable dict "`dockwatch_role_web_api_serverstransport`"
    ...
    ??? variable string "`dockwatch_role_web_http_port`"

        ```yaml
        # Type: string (quoted number)
        dockwatch_role_web_http_port:
        ```

    ??? variable string "`dockwatch_role_web_http_scheme`"
    ...
    ??? variable dict "`dockwatch_role_web_http_serverstransport`"
        ...
        ```yaml
        # Type: dict/omit
        dockwatch_role_web_http_serverstransport:
        ```
