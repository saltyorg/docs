    ...
    ??? variable dict "`semaphoreui_role_postgres_docker_healthcheck`"
        ...
        semaphoreui_role_postgres_docker_healthcheck:
          test:
            ...
            - "pg_isready"
            - "-d"
            - "{{ lookup('role_var', '_postgres_docker_env_db', role='semaphoreui') }}"
            - "-U"
            - "{{ lookup('role_var', '_postgres_user', role='semaphoreui') }}"
          start_period: 20s
          interval: 30s
          retries: 5
          timeout: 5s
...
=== "Web"
    ...
    ??? variable string "`semaphoreui_role_web_url`"
        ...
        ```yaml
        # Type: string
        semaphoreui_role_web_url: "{{ lookup('role_web', role='semaphoreui', scheme='https') }}"
        ```
...
=== "Traefik"
    ...
    ??? variable string "`semaphoreui_role_traefik_middleware_default_api`"

        ```yaml
        # Type: string
        semaphoreui_role_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

    ??? variable string "`semaphoreui_role_traefik_middleware_custom_api`"

        ```yaml
        # Type: string
        semaphoreui_role_traefik_middleware_custom_api: ""
        ```
...
=== "Docker+"
    ...
    ??? variable bool "`semaphoreui_role_docker_gpu_enabled`"
        ...
        # Type: bool (true/false)
        semaphoreui_role_docker_gpu_enabled: false
        ```

    ??? variable bool "`semaphoreui_role_docker_nvidia_disabled`"
        ...
        # Type: bool (true/false)
        semaphoreui_role_docker_nvidia_disabled: false
        ```

    ??? variable bool "`semaphoreui_role_docker_dev_dri_disabled`"
        ...
        # Type: bool (true/false)
        semaphoreui_role_docker_dev_dri_disabled: false
        ```
    ...
    ??? variable string "`semaphoreui_role_docker_stop_signal`"

        ```yaml
        # Type: string
        semaphoreui_role_docker_stop_signal:
        ```
...
=== "Global Override Options"
    ...
    ??? variable list "`semaphoreui_role_docker_networks_alias_custom`"

        ```yaml
        # Type: list
        semaphoreui_role_docker_networks_alias_custom:
        ```
