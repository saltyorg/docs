...
saltbox_automation:
  app_links:
    - name: Manual
      url: https://docs.linuxgsm.com
      type: documentation
      purpose: manual
    - name: Releases
      url: https://hub.docker.com/r/gameservermanagers/gameserver/tags
      type: docker
      purpose: release
    - name: Community
      url:
...
[:fontawesome-solid-book-open:**Manual**](https://docs.linuxgsm.com){ .md-button .md-button--stretch }
...
1.  **This role supports multiple instances via `lgsm_instances`.**
    ...
    !!! example "Example override"
        ...
        === "Instance-scoped"
            ...
            :material-arrow-right-bottom-bold: Applies to the instance named lgsm2
...
=== "Docker+"
    ...
    ??? variable bool "`lgsm_role_docker_gpu_enabled`{ .sb-show-on-unchecked }`lgsm2_docker_gpu_enabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        lgsm_role_docker_gpu_enabled: false
        ```
        ...
        # This setting does not install or enable GPU support on the server.
        # Type: bool (true/false)
        lgsm2_docker_gpu_enabled: false
        ```

    ??? variable bool "`lgsm_role_docker_nvidia_disabled`{ .sb-show-on-unchecked }`lgsm2_docker_nvidia_disabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        lgsm_role_docker_nvidia_disabled: false
        ```
        ...
        # Automatic /dev/dri access may remain.
        # Type: bool (true/false)
        lgsm2_docker_nvidia_disabled: false
        ```

    ??? variable bool "`lgsm_role_docker_dev_dri_disabled`{ .sb-show-on-unchecked }`lgsm2_docker_dev_dri_disabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        lgsm_role_docker_dev_dri_disabled: false
        ```
        ...
        # NVIDIA-specific access may remain.
        # Type: bool (true/false)
        lgsm2_docker_dev_dri_disabled: false
        ```
    ...
    ??? variable string "`lgsm_role_docker_stop_signal`{ .sb-show-on-unchecked }`lgsm2_docker_stop_signal`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        lgsm_role_docker_stop_signal:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        lgsm2_docker_stop_signal:
        ```
...
=== "Global Override Options"
    ...
    ??? variable list "`lgsm_role_paths_folders_list_custom`{ .sb-show-on-unchecked }`lgsm2_paths_folders_list_custom`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Extra directories to create
        # Type: list
        lgsm_role_paths_folders_list_custom:
        ```

        ```yaml { .sb-show-on-checked }
        # Extra directories to create
        # Type: list
        lgsm2_paths_folders_list_custom:
        ```

    ??? variable string "`lgsm_role_paths_group`{ .sb-show-on-unchecked }`lgsm2_paths_group`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Group for directories created by the role
        # Type: string
        lgsm_role_paths_group:
        ```
    ...
    ??? variable string "`lgsm_role_paths_owner`{ .sb-show-on-unchecked }`lgsm2_paths_owner`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Owner for directories created by the role
        # Type: string
        lgsm_role_paths_owner:
        ```
    ...
    ??? variable string "`lgsm_role_paths_permissions`{ .sb-show-on-unchecked }`lgsm2_paths_permissions`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Permissions for directories created by the role
        # Type: string
        lgsm_role_paths_permissions:
        ```

        ```yaml { .sb-show-on-checked }
        # Permissions for directories created by the role
        # Type: string
        lgsm2_paths_permissions:
        ```

    ??? variable bool "`lgsm_role_paths_recursive`{ .sb-show-on-unchecked }`lgsm2_paths_recursive`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Apply ownership and permissions recursively
        # Type: bool (true/false)
        lgsm_role_paths_recursive:
        ```

        ```yaml { .sb-show-on-checked }
        # Apply ownership and permissions recursively
        # Type: bool (true/false)
        lgsm2_paths_recursive:
        ```
