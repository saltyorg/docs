...
=== "Docker+"
    ...
    ??? variable string "`qbittorrent_role_docker_dev_dri`{ .sb-show-on-unchecked }`qbittorrent2_docker_dev_dri`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        qbittorrent_role_docker_dev_dri:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        qbittorrent2_docker_dev_dri:
        ```
