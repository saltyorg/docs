...
Redis is deployed using the official Alpine image with data persisting to `/opt/redis/`. Connect from other containers using `redis://redis:6379`.
...
=== "Docker+"
    ...
    ??? variable bool "`redis_role_docker_gpu_enabled`{ .sb-show-on-unchecked }`redis2_docker_gpu_enabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        redis_role_docker_gpu_enabled: false
        ```
        ...
        # This setting does not install or enable GPU support on the server.
        # Type: bool (true/false)
        redis2_docker_gpu_enabled: false
        ```

    ??? variable bool "`redis_role_docker_nvidia_disabled`{ .sb-show-on-unchecked }`redis2_docker_nvidia_disabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        redis_role_docker_nvidia_disabled: false
        ```
        ...
        # Automatic /dev/dri access may remain.
        # Type: bool (true/false)
        redis2_docker_nvidia_disabled: false
        ```

    ??? variable bool "`redis_role_docker_dev_dri_disabled`{ .sb-show-on-unchecked }`redis2_docker_dev_dri_disabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        redis_role_docker_dev_dri_disabled: false
        ```
        ...
        # NVIDIA-specific access may remain.
        # Type: bool (true/false)
        redis2_docker_dev_dri_disabled: false
        ```
    ...
    ??? variable string "`redis_role_docker_stop_signal`{ .sb-show-on-unchecked }`redis2_docker_stop_signal`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        redis_role_docker_stop_signal:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        redis2_docker_stop_signal:
        ```
...
=== "Global Override Options"
    ...
    ??? variable list "`redis_role_docker_networks_alias_custom`{ .sb-show-on-unchecked }`redis2_docker_networks_alias_custom`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: list
        redis_role_docker_networks_alias_custom:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: list
        redis2_docker_networks_alias_custom:
        ```
    ...
    ??? variable list "`redis_role_paths_folders_list_custom`{ .sb-show-on-unchecked }`redis2_paths_folders_list_custom`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Extra directories to create
        # Type: list
        redis_role_paths_folders_list_custom:
        ```

        ```yaml { .sb-show-on-checked }
        # Extra directories to create
        # Type: list
        redis2_paths_folders_list_custom:
        ```

    ??? variable string "`redis_role_paths_group`{ .sb-show-on-unchecked }`redis2_paths_group`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Group for directories created by the role
        # Type: string
        redis_role_paths_group:
        ```
    ...
    ??? variable string "`redis_role_paths_owner`{ .sb-show-on-unchecked }`redis2_paths_owner`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Owner for directories created by the role
        # Type: string
        redis_role_paths_owner:
        ```
    ...
    ??? variable string "`redis_role_paths_permissions`{ .sb-show-on-unchecked }`redis2_paths_permissions`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Permissions for directories created by the role
        # Type: string
        redis_role_paths_permissions:
        ```

        ```yaml { .sb-show-on-checked }
        # Permissions for directories created by the role
        # Type: string
        redis2_paths_permissions:
        ```

    ??? variable bool "`redis_role_paths_recursive`{ .sb-show-on-unchecked }`redis2_paths_recursive`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Apply owner and group recursively without changing child modes
        # Type: bool (true/false)
        redis_role_paths_recursive:
        ```

        ```yaml { .sb-show-on-checked }
        # Apply owner and group recursively without changing child modes
        # Type: bool (true/false)
        redis2_paths_recursive:
        ```
