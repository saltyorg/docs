...
=== "Docker+"
    ...
    ??? variable string "`maintainerr_role_docker_nvidia`{ .sb-show-on-unchecked }`maintainerr2_docker_nvidia`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        maintainerr_role_docker_nvidia:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        maintainerr2_docker_nvidia:
        ```
