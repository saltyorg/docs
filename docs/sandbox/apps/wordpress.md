...
=== "Web"
    ...
    ??? variable string "`wordpress_role_web_url`{ .sb-show-on-unchecked }`wordpress2_web_url`{ .sb-show-on-checked }"
        ...
        ```yaml { .sb-show-on-unchecked }
        # Type: string
        wordpress_role_web_url: "{{ lookup('role_web', role='wordpress', scheme='https') }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        wordpress2_web_url: "{{ lookup('role_web', role='wordpress', scheme='https') }}"
        ```
...
=== "Traefik"
    ...
    ??? variable string "`wordpress_role_traefik_middleware_default_api`{ .sb-show-on-unchecked }`wordpress2_traefik_middleware_default_api`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        wordpress_role_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        wordpress2_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

    ??? variable string "`wordpress_role_traefik_middleware_custom_api`{ .sb-show-on-unchecked }`wordpress2_traefik_middleware_custom_api`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        wordpress_role_traefik_middleware_custom_api: ""
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        wordpress2_traefik_middleware_custom_api: ""
        ```
...
=== "Global Override Options"
    ...
    ??? variable list "`wordpress_role_paths_folders_list_custom`{ .sb-show-on-unchecked }`wordpress2_paths_folders_list_custom`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Extra directories to create
        # Type: list
        wordpress_role_paths_folders_list_custom:
        ```

        ```yaml { .sb-show-on-checked }
        # Extra directories to create
        # Type: list
        wordpress2_paths_folders_list_custom:
        ```

    ??? variable string "`wordpress_role_paths_group`{ .sb-show-on-unchecked }`wordpress2_paths_group`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Group for directories created by the role
        # Type: string
        wordpress_role_paths_group:
        ```

        ```yaml { .sb-show-on-checked }
        # Group for directories created by the role
        # Type: string
        wordpress2_paths_group:
        ```

    ??? variable string "`wordpress_role_paths_owner`{ .sb-show-on-unchecked }`wordpress2_paths_owner`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Owner for directories created by the role
        # Type: string
        wordpress_role_paths_owner:
        ```

        ```yaml { .sb-show-on-checked }
        # Owner for directories created by the role
        # Type: string
        wordpress2_paths_owner:
        ```

    ??? variable string "`wordpress_role_paths_permissions`{ .sb-show-on-unchecked }`wordpress2_paths_permissions`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Permissions for directories created by the role
        # Type: string
        wordpress_role_paths_permissions:
        ```

        ```yaml { .sb-show-on-checked }
        # Permissions for directories created by the role
        # Type: string
        wordpress2_paths_permissions:
        ```
    ...
    ??? variable bool "`wordpress_role_paths_recursive`{ .sb-show-on-unchecked }`wordpress2_paths_recursive`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        wordpress_role_paths_recursive:
        ```

        ```yaml { .sb-show-on-checked }
        # Apply owner and group recursively without changing child modes
        # Type: bool (true/false)
        wordpress2_paths_recursive:
        ```
