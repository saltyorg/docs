...
=== "Docker+"
    ...
    ??? variable string "`tautulli_role_docker_nvidia`{ .sb-show-on-unchecked }`tautulli2_docker_nvidia`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        tautulli_role_docker_nvidia:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        tautulli2_docker_nvidia:
        ```
