...
=== "Docker+"
    ...
    ??? variable string "`bazarr_role_docker_dev_dri`{ .sb-show-on-unchecked }`bazarr2_docker_dev_dri`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        bazarr_role_docker_dev_dri:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        bazarr2_docker_dev_dri:
        ```
