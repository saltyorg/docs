...
=== "Docker+"
    ...
    ??? variable string "`postgres_role_docker_nvidia`{ .sb-show-on-unchecked }`postgres2_docker_nvidia`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        postgres_role_docker_nvidia:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        postgres2_docker_nvidia:
        ```
