---
icon: material/docker
title: Calibre-Web
hide:
  - tags
tags:
  - calibre
  - ebooks
  - reading
saltbox_automation:
  app_links:
    - name: Manual
      url: https://github.com/janeczku/calibre-web/wiki
      type: documentation
      purpose: manual
    - name: Releases
      url: https://hub.docker.com/r/linuxserver/calibre-web/tags
      type: docker
      purpose: release
    - name: Community
      url: https://discord.gg/h2VsJ2NEfB
      type: discord
      purpose: community
  project_description:
    name: Calibre-Web
    summary: |-
      a web app providing a clean interface for browsing, reading and downloading eBooks using an existing Calibre database.
    link: https://github.com/janeczku/calibre-web
---

<!-- BEGIN SALTBOX MANAGED OVERVIEW SECTION -->
<!-- This section is managed by sb-docs - DO NOT EDIT MANUALLY -->
# Calibre-Web

## Overview

[Calibre-Web](https://github.com/janeczku/calibre-web) is a web app providing a clean interface for browsing, reading and downloading eBooks using an existing Calibre database.

<div class="grid grid--buttons" markdown data-search-exclude>

[:fontawesome-solid-book-open:**Manual**](https://github.com/janeczku/calibre-web/wiki){ .md-button .md-button--stretch }

[:fontawesome-brands-docker:**Releases**](https://hub.docker.com/r/linuxserver/calibre-web/tags){ .md-button .md-button--stretch }

[:fontawesome-brands-discord:**Community**](https://discord.gg/h2VsJ2NEfB){ .md-button .md-button--stretch }

</div>

---
<!-- END SALTBOX MANAGED OVERVIEW SECTION -->

!!! info
    Calibre and Calibre-web do NOT need to be on the same server. But you do need to have a local copy of the Calibre `metadata.db` and a path to the books for calibre-web to operate.

## Deployment

```shell
sb install sandbox-calibre-web
```

## Usage

Visit <https://calibre-web.iYOUR_DOMAIN_NAMEi>.

- Default admin login:

  ```yaml
  Username: admin
  Password: admin123
  ```

  Change the default log in details immediately.

- **Unrar** is included by default and needs to be set in the Calibre-Web admin page (Basic Configuration:External Binaries) with a path of `/usr/bin/unrar`.

- Automatic ebook conversion via Calibre converter is included. Enable it in the Calibre-Web admin page (Basic Configuration:External Binaries) by setting the Path to Calibre E-Book Converter to `/usr/bin/ebook-convert`.

- The **kepubify ebook conversion tool** (MIT License) to convert epub to kepub is included. In the Calibre-Web admin page (Basic Configuration:External Binaries) set the Path to Kepubify E-Book Converter to `/usr/bin/kepubify`.

- Book cover thumbnails will improve browsing speed but they aren't generated by default. Enable them on the Calibre-Web admin page by either schedule a task (Edit Scheduled Task Settings) or running it manually (Refresh Thumbnail Cover Cache).

Useful docker commands

  ```shell title="Shell access whilst the container is running:"
  docker exec -it calibre-web /bin/bash
  ```

  ```shell title="To monitor the logs of the container in realtime:"
  docker logs -f calibre-web
  ```

  ```shell title="Container version number:"
  docker inspect -f '{{ index .Config.Labels "build_version" }}' calibre-web
  ```

  ```shell title="Image version number:"
  docker inspect -f '{{ index .Config.Labels "build_version" }}' linuxserver/calibre-web
  ```

### SK's Calibre-Web Usage Tips

#### SMTP Email Server Setup

A useful function of Calibre-Web is sending ebooks by email. Therefore, you need to set up SMTP e-mail server settings.

I am using Google to host the email for my domain name. In my case, after trial and error, I found it most reliable to go into my Google control panel, go to the SMTP settings, and whitelist my server’s IP address without authentication. Now, I can send email from any Saltbox app that supports it (Ombi, Tautelli, Organizr, and Calibre-Web) with no troubles. These settings can be found under `Apps > Google Workspace > Settings for Gmail > Routing` and the setting you need to add or change is `SMTP relay service`. or just search `SMTP` in the search bar at the top of the Google admin console, and find `SMTP relay service` from there.

```text
Hostname: smtp-relay.gmail.com, Port:  25,  SSL: No
```

##### Kindle Setup

There is a nice benefit to using a Kindle. When you send a “personal document” (book) to your Kindle account, it will automatically download to the specific device tied to the email address you use. Even though it’s called a document, it goes into your regular library and looks just like any book you may have purchased, including the cover. In addition, that book is available on any other device or app tied to your account, so you can read the same book on your phone, a Kindle Paperwhite, and an iPad. Amazon’s WhisperSync feature will bookmark your most recent page in the background, so if you switch devices it will ask if you want to update to the most recent page read.

If you (or your users) want to have books sent directly to a Kindle from Calibre-Web email, then there are additional one-time setup steps for each user.

- **Tell Amazon to accept books sent from your website**
  - Go to <www.amazon.com>
  - On the top navigation bar, go to `Account & Lists`. In the dropdown, click on `Your Content and Devices`.
  - Towards the top of the white section, in the middle, click on `Preferences`
  - Scroll down to Personal Document Settings. Click the title to open up that section of the page.
  - Under `Approved Personal Document E-mail List`, click the link for `"Add a new approved e-mail address"`
  - In the popup, add `@xYOUR_DOMAIN_NAMEx` and save. Done!

    Before closing the website, you might want to grab your device email address for the next step. Under the Send-to-Kindle E-Mail Settings, copy the email address where you want the books sent by default.

- **Add your Kindle email address to your profile on books.xYOUR_DOMAIN_NAMEx**
  - Once logged in, on the top ride side, click your name to open your profile
  - Add your kindle email address and save
  - Your Kindle email address will be something like <name_79@kindle.com>. Every Kindle device and mobile app has its own unique address, however, you can send to any of your devices and it will still be available on all of them. You can find this email address either in the Settings of the device/app, or copy it from the Devices page

!!! info
    Kindle has started sending verification emails on every document sent if your Kindle's email is the one they generated for you. While you are on your settings page, go ahead and make up a new Kindle email address.

Enjoy!

### Advanced Method of Library Storage

Since only the `metadata.db` file has to be local, you can keep the metadata.db file in `/mnt/local/Media/Books` as **RW** and the actual book files in `teamdrive:Books` as **RO** and mergerfs them together. Use the latest rclone `--vfs-cache-mode=full` and related cloud-seeding settings for your teamdrive mount so that it does not get laggy. Have a script that copies the metadata.db file regularly to the local disk, and leave the books in the cloud.

If this paragraph does not make sense to you, then please do not try it.

<!-- BEGIN SALTBOX MANAGED VARIABLES SECTION -->
<!-- This section is managed by sb-docs - DO NOT EDIT MANUALLY -->
## Role Defaults

Variables can be customized using the [Inventory](/saltbox/inventory/index.md#overriding-variables){ data-preview }. <span title="View override specifics for this role" markdown>(1)</span>
{ .annotate .sb-annotated }

1.  !!! example "Example override"

        ```yaml
        calibre_web_name: "custom_value"
        ```

    !!! warning "Avoid overriding variables ending in `_default`"

        When overriding variables that end in `_default` (like `calibre_web_docker_envs_default`), you replace the entire default configuration. Future updates that add new default values will not be applied to your setup, potentially breaking functionality.

        Instead, use the corresponding `_custom` variable (like `calibre_web_docker_envs_custom`) to add your changes. Custom values are merged with defaults, ensuring you receive updates.

=== "Basics"

    ??? variable string "`calibre_web_name`"

        ```yaml
        # Type: string
        calibre_web_name: calibre-web
        ```

=== "Web"

    ??? variable string "`calibre_web_role_web_subdomain`"

        ```yaml
        # Type: string
        calibre_web_role_web_subdomain: "{{ calibre_web_name }}"
        ```

    ??? variable string "`calibre_web_role_web_domain`"

        ```yaml
        # Type: string
        calibre_web_role_web_domain: "{{ user.domain }}"
        ```

    ??? variable string "`calibre_web_role_web_port`"

        ```yaml
        # Type: string
        calibre_web_role_web_port: "8083"
        ```

    ??? variable string "`calibre_web_role_web_url`"

        ```yaml
        # Type: string
        calibre_web_role_web_url: "{{ lookup('role_web', role='calibre_web', scheme='https') }}"
        ```

=== "DNS"

    ??? variable string "`calibre_web_role_dns_record`"

        ```yaml
        # Type: string
        calibre_web_role_dns_record: "{{ lookup('role_var', '_web_subdomain', role='calibre_web') }}"
        ```

    ??? variable string "`calibre_web_role_dns_zone`"

        ```yaml
        # Type: string
        calibre_web_role_dns_zone: "{{ lookup('role_var', '_web_domain', role='calibre_web') }}"
        ```

    ??? variable bool "`calibre_web_role_dns_proxy`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_dns_proxy: "{{ dns_proxied }}"
        ```

=== "Traefik"

    ??? variable string "`calibre_web_role_traefik_sso_middleware`"

        ```yaml
        # Type: string
        calibre_web_role_traefik_sso_middleware: ""
        ```

    ??? variable string "`calibre_web_role_traefik_middleware_default`"

        ```yaml
        # Type: string
        calibre_web_role_traefik_middleware_default: "{{ traefik_default_middleware }},kobo-sync-headers"
        ```

    ??? variable string "`calibre_web_role_traefik_middleware_custom`"

        ```yaml
        # Type: string
        calibre_web_role_traefik_middleware_custom: ""
        ```

    ??? variable string "`calibre_web_role_traefik_middleware_default_api`"

        ```yaml
        # Type: string
        calibre_web_role_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

    ??? variable string "`calibre_web_role_traefik_middleware_custom_api`"

        ```yaml
        # Type: string
        calibre_web_role_traefik_middleware_custom_api: ""
        ```

    ??? variable string "`calibre_web_role_traefik_certresolver`"

        ```yaml
        # Type: string
        calibre_web_role_traefik_certresolver: "{{ traefik_default_certresolver }}"
        ```

    ??? variable bool "`calibre_web_role_traefik_enabled`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_traefik_enabled: true
        ```

    ??? variable bool "`calibre_web_role_traefik_api_enabled`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_traefik_api_enabled: false
        ```

    ??? variable string "`calibre_web_role_traefik_api_endpoint`"

        ```yaml
        # Type: string
        calibre_web_role_traefik_api_endpoint: ""
        ```

=== "Theme"

    ??? variable bool "`calibre_web_role_themepark_enabled`"

        ```yaml
        # Options can be found at https://github.com/themepark-dev/theme.park
        # Type: bool (true/false)
        calibre_web_role_themepark_enabled: false
        ```

    ??? variable string "`calibre_web_role_themepark_theme`"

        ```yaml
        # Type: string
        calibre_web_role_themepark_theme: "{{ global_themepark_theme }}"
        ```

    ??? variable string "`calibre_web_role_themepark_domain`"

        ```yaml
        # Type: string
        calibre_web_role_themepark_domain: "{{ global_themepark_domain }}"
        ```

=== "Docker"

    <h5>Container</h5>

    ??? variable string "`calibre_web_role_docker_container`"

        ```yaml
        # Type: string
        calibre_web_role_docker_container: "{{ calibre_web_name }}"
        ```

    <h5>Image</h5>

    ??? variable bool "`calibre_web_role_docker_image_pull`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_image_pull: true
        ```

    ??? variable string "`calibre_web_role_docker_image_repo`"

        ```yaml
        # Type: string
        calibre_web_role_docker_image_repo: "lscr.io/linuxserver/calibre-web"
        ```

    ??? variable string "`calibre_web_role_docker_image_tag`"

        ```yaml
        # Type: string
        calibre_web_role_docker_image_tag: "latest"
        ```

    ??? variable string "`calibre_web_role_docker_image`"

        ```yaml
        # Type: string
        calibre_web_role_docker_image: "{{ lookup('role_var', '_docker_image_repo', role='calibre_web') }}:{{ lookup('role_var', '_docker_image_tag', role='calibre_web') }}"
        ```

    <h5>Envs</h5>

    ??? variable dict "`calibre_web_role_docker_envs_default`"

        ```yaml
        # Type: dict
        calibre_web_role_docker_envs_default:
          PUID: "{{ uid }}"
          PGID: "{{ gid }}"
          TZ: "{{ tz }}"
          CACHE_DIR: "/cache/"
          USE_CONFIG_DIR: "true"
          DOCKER_MODS: linuxserver/mods:universal-calibre
        ```

    ??? variable dict "`calibre_web_role_docker_envs_theme`"

        ```yaml
        # Type: dict
        calibre_web_role_docker_envs_theme:
          DOCKER_MODS: "linuxserver/mods:universal-calibre|ghcr.io/themepark-dev/theme.park:calibre-web"
          TP_DOMAIN: "{{ lookup('role_var', '_themepark_domain', role='calibre_web') }}"
          TP_THEME: "{{ lookup('role_var', '_themepark_theme', role='calibre_web') }}"
        ```

    ??? variable dict "`calibre_web_role_docker_envs_custom`"

        ```yaml
        # Type: dict
        calibre_web_role_docker_envs_custom: {}
        ```

    <h5>Volumes</h5>

    ??? variable list "`calibre_web_role_docker_volumes_default`"

        ```yaml
        # Type: list
        calibre_web_role_docker_volumes_default:
          - "{{ lookup('role_var', '_paths_location', role='calibre_web') }}:/calibre-web"
          - "{{ lookup('role_var', '_paths_location', role='calibre_web') }}/config:/config"
          - "{{ lookup('role_var', '_paths_location', role='calibre_web') }}/cache:/cache"
          - "/mnt/unionfs/Media/Books:/books"
        ```

    ??? variable list "`calibre_web_role_docker_volumes_custom`"

        ```yaml
        # Type: list
        calibre_web_role_docker_volumes_custom: []
        ```

    <h5>Labels</h5>

    ??? variable dict "`calibre_web_role_docker_labels_default`"

        ```yaml
        # Type: dict
        calibre_web_role_docker_labels_default:
          traefik.http.middlewares.kobo-sync-headers.headers.customrequestheaders.X-Scheme: "https"
        ```

    ??? variable dict "`calibre_web_role_docker_labels_custom`"

        ```yaml
        # Type: dict
        calibre_web_role_docker_labels_custom: {}
        ```

    <h5>Hostname</h5>

    ??? variable string "`calibre_web_role_docker_hostname`"

        ```yaml
        # Type: string
        calibre_web_role_docker_hostname: "{{ calibre_web_name }}"
        ```

    <h5>Networks</h5>

    ??? variable string "`calibre_web_role_docker_networks_alias`"

        ```yaml
        # Type: string
        calibre_web_role_docker_networks_alias: "{{ calibre_web_name }}"
        ```

    ??? variable list "`calibre_web_role_docker_networks_default`"

        ```yaml
        # Type: list
        calibre_web_role_docker_networks_default: []
        ```

    ??? variable list "`calibre_web_role_docker_networks_custom`"

        ```yaml
        # Type: list
        calibre_web_role_docker_networks_custom: []
        ```

    <h5>Restart Policy</h5>

    ??? variable string "`calibre_web_role_docker_restart_policy`"

        ```yaml
        # Type: string
        calibre_web_role_docker_restart_policy: unless-stopped
        ```

=== "Docker+"

    The following advanced options are available via create_docker_container but are not defined in the role. See: [docker_container module](https://docs.ansible.com/ansible/latest/collections/community/docker/docker_container_module.html)

    A blank value is YAML null and inherits any lower-precedence role or shared default. Explicit Ansible omit is accepted only for optional Docker settings; default-backed and required settings reject it. Use the documented typed empty value, such as `""`, `[]`, or `{}`, when disabling a guaranteed setting.

    <h5>GPU</h5>

    ??? variable bool "`calibre_web_role_docker_gpu_enabled`"

        ```yaml
        # Set this to true to let the app use a GPU.
        # Intel access also requires gpu.intel: true.
        # NVIDIA access also requires nvidia_enabled: true.
        # This setting does not install or enable GPU support on the server.
        # Type: bool (true/false)
        calibre_web_role_docker_gpu_enabled: false
        ```

    ??? variable bool "`calibre_web_role_docker_nvidia_disabled`"

        ```yaml
        # Set this to true to turn off automatic NVIDIA access for this app.
        # It only has an effect when the app's _docker_gpu_enabled option and
        # nvidia_enabled are both true.
        # Automatic /dev/dri access may remain.
        # Type: bool (true/false)
        calibre_web_role_docker_nvidia_disabled: false
        ```

    ??? variable bool "`calibre_web_role_docker_dev_dri_disabled`"

        ```yaml
        # Set this to true to stop Saltbox from automatically sharing the
        # server's /dev/dri video devices with this app.
        # It only has an effect when the app's _docker_gpu_enabled option is true
        # and either gpu.intel or nvidia_enabled is true.
        # NVIDIA-specific access may remain.
        # Type: bool (true/false)
        calibre_web_role_docker_dev_dri_disabled: false
        ```

    <h5>Resource Limits</h5>

    ??? variable int "`calibre_web_role_docker_blkio_weight`"

        ```yaml
        # Type: int
        calibre_web_role_docker_blkio_weight:
        ```

    ??? variable int "`calibre_web_role_docker_cpu_period`"

        ```yaml
        # Type: int
        calibre_web_role_docker_cpu_period:
        ```

    ??? variable int "`calibre_web_role_docker_cpu_quota`"

        ```yaml
        # Type: int
        calibre_web_role_docker_cpu_quota:
        ```

    ??? variable int "`calibre_web_role_docker_cpu_shares`"

        ```yaml
        # Type: int
        calibre_web_role_docker_cpu_shares:
        ```

    ??? variable string "`calibre_web_role_docker_cpus`"

        ```yaml
        # CPU allocation accepted as a numeric string, such as 1.5
        # Type: string (quoted number)
        calibre_web_role_docker_cpus:
        ```

    ??? variable string "`calibre_web_role_docker_cpuset_cpus`"

        ```yaml
        # Type: string
        calibre_web_role_docker_cpuset_cpus:
        ```

    ??? variable string "`calibre_web_role_docker_cpuset_mems`"

        ```yaml
        # Type: string
        calibre_web_role_docker_cpuset_mems:
        ```

    ??? variable string "`calibre_web_role_docker_kernel_memory`"

        ```yaml
        # Type: string
        calibre_web_role_docker_kernel_memory:
        ```

    ??? variable string "`calibre_web_role_docker_memory`"

        ```yaml
        # Type: string
        calibre_web_role_docker_memory:
        ```

    ??? variable string "`calibre_web_role_docker_memory_reservation`"

        ```yaml
        # Type: string
        calibre_web_role_docker_memory_reservation:
        ```

    ??? variable string "`calibre_web_role_docker_memory_swap`"

        ```yaml
        # Type: string
        calibre_web_role_docker_memory_swap:
        ```

    ??? variable int "`calibre_web_role_docker_memory_swappiness`"

        ```yaml
        # Type: int
        calibre_web_role_docker_memory_swappiness:
        ```

    ??? variable string "`calibre_web_role_docker_shm_size`"

        ```yaml
        # Type: string
        calibre_web_role_docker_shm_size:
        ```

    <h5>Security & Devices</h5>

    ??? variable list "`calibre_web_role_docker_cap_drop`"

        ```yaml
        # Type: list
        calibre_web_role_docker_cap_drop:
        ```

    ??? variable string "`calibre_web_role_docker_cgroupns_mode`"

        ```yaml
        # Type: string
        calibre_web_role_docker_cgroupns_mode:
        ```

    ??? variable list "`calibre_web_role_docker_device_cgroup_rules`"

        ```yaml
        # Type: list
        calibre_web_role_docker_device_cgroup_rules:
        ```

    ??? variable list "`calibre_web_role_docker_device_read_bps`"

        ```yaml
        # Type: list
        calibre_web_role_docker_device_read_bps:
        ```

    ??? variable list "`calibre_web_role_docker_device_read_iops`"

        ```yaml
        # Type: list
        calibre_web_role_docker_device_read_iops:
        ```

    ??? variable list "`calibre_web_role_docker_device_requests`"

        ```yaml
        # Type: list
        calibre_web_role_docker_device_requests:
        ```

    ??? variable list "`calibre_web_role_docker_device_write_bps`"

        ```yaml
        # Type: list
        calibre_web_role_docker_device_write_bps:
        ```

    ??? variable list "`calibre_web_role_docker_device_write_iops`"

        ```yaml
        # Type: list
        calibre_web_role_docker_device_write_iops:
        ```

    ??? variable list "`calibre_web_role_docker_devices`"

        ```yaml
        # Type: list
        calibre_web_role_docker_devices:
        ```

    ??? variable list "`calibre_web_role_docker_groups`"

        ```yaml
        # Type: list
        calibre_web_role_docker_groups:
        ```

    ??? variable bool "`calibre_web_role_docker_privileged`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_privileged:
        ```

    ??? variable list "`calibre_web_role_docker_security_opts`"

        ```yaml
        # Type: list
        calibre_web_role_docker_security_opts:
        ```

    ??? variable string "`calibre_web_role_docker_user`"

        ```yaml
        # Type: string
        calibre_web_role_docker_user:
        ```

    ??? variable string "`calibre_web_role_docker_userns_mode`"

        ```yaml
        # Type: string
        calibre_web_role_docker_userns_mode:
        ```

    <h5>Networking</h5>

    ??? variable list "`calibre_web_role_docker_dns_opts`"

        ```yaml
        # Type: list
        calibre_web_role_docker_dns_opts:
        ```

    ??? variable list "`calibre_web_role_docker_dns_search_domains`"

        ```yaml
        # Type: list
        calibre_web_role_docker_dns_search_domains:
        ```

    ??? variable list "`calibre_web_role_docker_dns_servers`"

        ```yaml
        # Type: list
        calibre_web_role_docker_dns_servers:
        ```

    ??? variable string "`calibre_web_role_docker_domainname`"

        ```yaml
        # Type: string
        calibre_web_role_docker_domainname:
        ```

    ??? variable list "`calibre_web_role_docker_exposed_ports`"

        ```yaml
        # Type: list
        calibre_web_role_docker_exposed_ports:
        ```

    ??? variable dict "`calibre_web_role_docker_hosts`"

        ```yaml
        # Type: dict
        calibre_web_role_docker_hosts:
        ```

    ??? variable bool "`calibre_web_role_docker_hosts_use_common`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_hosts_use_common:
        ```

    ??? variable string "`calibre_web_role_docker_ipc_mode`"

        ```yaml
        # Type: string
        calibre_web_role_docker_ipc_mode:
        ```

    ??? variable list "`calibre_web_role_docker_links`"

        ```yaml
        # Type: list
        calibre_web_role_docker_links:
        ```

    ??? variable string "`calibre_web_role_docker_network_mode`"

        ```yaml
        # Type: string
        calibre_web_role_docker_network_mode:
        ```

    ??? variable string "`calibre_web_role_docker_pid_mode`"

        ```yaml
        # Type: string
        calibre_web_role_docker_pid_mode:
        ```

    ??? variable list "`calibre_web_role_docker_ports`"

        ```yaml
        # Type: list
        calibre_web_role_docker_ports:
        ```

    ??? variable string "`calibre_web_role_docker_uts`"

        ```yaml
        # Type: string
        calibre_web_role_docker_uts:
        ```

    <h5>Storage</h5>

    ??? variable bool "`calibre_web_role_docker_keep_volumes`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_keep_volumes:
        ```

    ??? variable list "`calibre_web_role_docker_mounts`"

        ```yaml
        # Type: list
        calibre_web_role_docker_mounts:
        ```

    ??? variable dict "`calibre_web_role_docker_storage_opts`"

        ```yaml
        # Type: dict
        calibre_web_role_docker_storage_opts:
        ```

    ??? variable list "`calibre_web_role_docker_tmpfs`"

        ```yaml
        # Type: list
        calibre_web_role_docker_tmpfs:
        ```

    ??? variable string "`calibre_web_role_docker_volume_driver`"

        ```yaml
        # Type: string
        calibre_web_role_docker_volume_driver:
        ```

    ??? variable list "`calibre_web_role_docker_volumes_from`"

        ```yaml
        # Type: list
        calibre_web_role_docker_volumes_from:
        ```

    ??? variable bool "`calibre_web_role_docker_volumes_global`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_volumes_global:
        ```

    ??? variable string "`calibre_web_role_docker_working_dir`"

        ```yaml
        # Type: string
        calibre_web_role_docker_working_dir:
        ```

    <h5>Monitoring & Lifecycle</h5>

    ??? variable bool "`calibre_web_role_docker_auto_remove`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_auto_remove:
        ```

    ??? variable bool "`calibre_web_role_docker_cleanup`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_cleanup:
        ```

    ??? variable bool "`calibre_web_role_docker_force_kill`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_force_kill:
        ```

    ??? variable dict "`calibre_web_role_docker_healthcheck`"

        ```yaml
        # Type: dict
        calibre_web_role_docker_healthcheck:
        ```

    ??? variable int "`calibre_web_role_docker_healthy_wait_timeout`"

        ```yaml
        # Healthy-state wait timeout in seconds
        # Type: int
        calibre_web_role_docker_healthy_wait_timeout:
        ```

    ??? variable bool "`calibre_web_role_docker_init`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_init:
        ```

    ??? variable string "`calibre_web_role_docker_kill_signal`"

        ```yaml
        # Type: string
        calibre_web_role_docker_kill_signal:
        ```

    ??? variable string "`calibre_web_role_docker_log_driver`"

        ```yaml
        # Type: string
        calibre_web_role_docker_log_driver:
        ```

    ??? variable dict "`calibre_web_role_docker_log_options`"

        ```yaml
        # Type: dict
        calibre_web_role_docker_log_options:
        ```

    ??? variable bool "`calibre_web_role_docker_oom_killer`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_oom_killer:
        ```

    ??? variable int "`calibre_web_role_docker_oom_score_adj`"

        ```yaml
        # Type: int
        calibre_web_role_docker_oom_score_adj:
        ```

    ??? variable bool "`calibre_web_role_docker_output_logs`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_output_logs:
        ```

    ??? variable bool "`calibre_web_role_docker_paused`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_paused:
        ```

    ??? variable bool "`calibre_web_role_docker_recreate`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_recreate:
        ```

    ??? variable int "`calibre_web_role_docker_restart_retries`"

        ```yaml
        # Type: int
        calibre_web_role_docker_restart_retries:
        ```

    ??? variable string "`calibre_web_role_docker_stop_signal`"

        ```yaml
        # Type: string
        calibre_web_role_docker_stop_signal:
        ```

    ??? variable int "`calibre_web_role_docker_stop_timeout`"

        ```yaml
        # Type: int
        calibre_web_role_docker_stop_timeout:
        ```

    <h5>Other Options</h5>

    ??? variable list "`calibre_web_role_docker_capabilities`"

        ```yaml
        # Type: list
        calibre_web_role_docker_capabilities:
        ```

    ??? variable string "`calibre_web_role_docker_cgroup_parent`"

        ```yaml
        # Type: string
        calibre_web_role_docker_cgroup_parent:
        ```

    ??? variable list "`calibre_web_role_docker_commands`"

        ```yaml
        # Type: list
        calibre_web_role_docker_commands:
        ```

    ??? variable int "`calibre_web_role_docker_create_timeout`"

        ```yaml
        # Type: int
        calibre_web_role_docker_create_timeout:
        ```

    ??? variable list "`calibre_web_role_docker_entrypoint`"

        ```yaml
        # Type: list
        calibre_web_role_docker_entrypoint:
        ```

    ??? variable string "`calibre_web_role_docker_env_file`"

        ```yaml
        # Type: string
        calibre_web_role_docker_env_file:
        ```

    ??? variable bool "`calibre_web_role_docker_labels_use_common`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_labels_use_common:
        ```

    ??? variable bool "`calibre_web_role_docker_read_only`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_read_only:
        ```

    ??? variable string "`calibre_web_role_docker_runtime`"

        ```yaml
        # Type: string
        calibre_web_role_docker_runtime:
        ```

    ??? variable dict "`calibre_web_role_docker_sysctls`"

        ```yaml
        # Type: dict
        calibre_web_role_docker_sysctls:
        ```

    ??? variable list "`calibre_web_role_docker_ulimits`"

        ```yaml
        # Type: list
        calibre_web_role_docker_ulimits:
        ```

=== "Global Override Options"

    ??? variable bool "`calibre_web_role_autoheal_enabled`"

        ```yaml
        # Enable or disable Autoheal monitoring for the container created when deploying
        # Type: bool (true/false)
        calibre_web_role_autoheal_enabled: true
        ```

    ??? variable string "`calibre_web_role_depends_on`"

        ```yaml
        # List of container dependencies that must be running before the container start
        # Type: string
        calibre_web_role_depends_on: ""
        ```

    ??? variable string "`calibre_web_role_depends_on_delay`"

        ```yaml
        # Delay in seconds before starting the container after dependencies are ready
        # Type: string (quoted number)
        calibre_web_role_depends_on_delay: "0"
        ```

    ??? variable string "`calibre_web_role_depends_on_healthchecks`"

        ```yaml
        # Enable healthcheck waiting for container dependencies
        # Type: string ("true"/"false")
        calibre_web_role_depends_on_healthchecks:
        ```

    ??? variable bool "`calibre_web_role_diun_enabled`"

        ```yaml
        # Enable or disable Diun update notifications for the container created when deploying
        # Type: bool (true/false)
        calibre_web_role_diun_enabled: true
        ```

    ??? variable bool "`calibre_web_role_dns_enabled`"

        ```yaml
        # Enable or disable automatic DNS record creation for the container
        # Type: bool (true/false)
        calibre_web_role_dns_enabled: true
        ```

    ??? variable bool "`calibre_web_role_docker_controller`"

        ```yaml
        # Enable or disable Saltbox Docker Controller management for the container
        # Type: bool (true/false)
        calibre_web_role_docker_controller: true
        ```

    ??? variable list "`calibre_web_role_docker_networks_alias_custom`"

        ```yaml
        # Type: list
        calibre_web_role_docker_networks_alias_custom:
        ```

    ??? variable bool "`calibre_web_role_docker_volumes_download`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_docker_volumes_download:
        ```

    ??? variable list "`calibre_web_role_paths_folders_list_custom`"

        ```yaml
        # Extra directories to create
        # Type: list
        calibre_web_role_paths_folders_list_custom:
        ```

    ??? variable string "`calibre_web_role_paths_group`"

        ```yaml
        # Group for directories created by the role
        # Type: string
        calibre_web_role_paths_group:
        ```

    ??? variable string "`calibre_web_role_paths_owner`"

        ```yaml
        # Owner for directories created by the role
        # Type: string
        calibre_web_role_paths_owner:
        ```

    ??? variable string "`calibre_web_role_paths_permissions`"

        ```yaml
        # Permissions for directories created by the role
        # Type: string
        calibre_web_role_paths_permissions:
        ```

    ??? variable bool "`calibre_web_role_paths_recursive`"

        ```yaml
        # Apply owner and group recursively without changing child modes
        # Type: bool (true/false)
        calibre_web_role_paths_recursive:
        ```

    ??? variable list "`calibre_web_role_themepark_addons`"

        ```yaml
        # ThemePark addon names to enable
        # Type: list
        calibre_web_role_themepark_addons:
        ```

    ??? variable string "`calibre_web_role_themepark_app`"

        ```yaml
        # Type: string
        calibre_web_role_themepark_app:
        ```

    ??? variable string "`calibre_web_role_traefik_api_middleware_http`"

        ```yaml
        # Type: string
        calibre_web_role_traefik_api_middleware_http:
        ```

    ??? variable bool "`calibre_web_role_traefik_autodetect_enabled`"

        ```yaml
        # Enable Traefik autodetect middleware for the container
        # Type: bool (true/false)
        calibre_web_role_traefik_autodetect_enabled: false
        ```

    ??? variable bool "`calibre_web_role_traefik_crowdsec_enabled`"

        ```yaml
        # Enable CrowdSec middleware for the container
        # Type: bool (true/false)
        calibre_web_role_traefik_crowdsec_enabled: false
        ```

    ??? variable bool "`calibre_web_role_traefik_error_pages_enabled`"

        ```yaml
        # Enable custom error pages middleware for the container
        # Type: bool (true/false)
        calibre_web_role_traefik_error_pages_enabled: false
        ```

    ??? variable bool "`calibre_web_role_traefik_gzip_enabled`"

        ```yaml
        # Enable gzip compression middleware for the container
        # Type: bool (true/false)
        calibre_web_role_traefik_gzip_enabled: false
        ```

    ??? variable string "`calibre_web_role_traefik_middleware_http`"

        ```yaml
        # Type: string
        calibre_web_role_traefik_middleware_http:
        ```

    ??? variable bool "`calibre_web_role_traefik_middleware_http_api_insecure`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_traefik_middleware_http_api_insecure:
        ```

    ??? variable bool "`calibre_web_role_traefik_middleware_http_insecure`"

        ```yaml
        # Type: bool (true/false)
        calibre_web_role_traefik_middleware_http_insecure:
        ```

    ??? variable string "`calibre_web_role_traefik_priority`"

        ```yaml
        # Type: string
        calibre_web_role_traefik_priority:
        ```

    ??? variable bool "`calibre_web_role_traefik_robot_enabled`"

        ```yaml
        # Enable robots.txt middleware for the container
        # Type: bool (true/false)
        calibre_web_role_traefik_robot_enabled: true
        ```

    ??? variable bool "`calibre_web_role_traefik_tailscale_enabled`"

        ```yaml
        # Enable Tailscale-specific Traefik configuration for the container
        # Type: bool (true/false)
        calibre_web_role_traefik_tailscale_enabled: false
        ```

    ??? variable bool "`calibre_web_role_traefik_wildcard_enabled`"

        ```yaml
        # Enable wildcard certificate for the container
        # Type: bool (true/false)
        calibre_web_role_traefik_wildcard_enabled: true
        ```

    ??? variable string "`calibre_web_role_web_api_http_port`"

        ```yaml
        # Type: string (quoted number)
        calibre_web_role_web_api_http_port:
        ```

    ??? variable string "`calibre_web_role_web_api_http_scheme`"

        ```yaml
        # Type: string ("http"/"https")
        calibre_web_role_web_api_http_scheme:
        ```

    ??? variable dict "`calibre_web_role_web_api_http_serverstransport`"

        ```yaml
        # Type: dict/omit
        calibre_web_role_web_api_http_serverstransport:
        ```

    ??? variable string "`calibre_web_role_web_api_port`"

        ```yaml
        # Type: string (quoted number)
        calibre_web_role_web_api_port:
        ```

    ??? variable string "`calibre_web_role_web_api_scheme`"

        ```yaml
        # Type: string ("http"/"https")
        calibre_web_role_web_api_scheme:
        ```

    ??? variable dict "`calibre_web_role_web_api_serverstransport`"

        ```yaml
        # Type: dict/omit
        calibre_web_role_web_api_serverstransport:
        ```

    ??? variable list "`calibre_web_role_web_fqdn_override`"

        ```yaml
        # Override the Traefik fully qualified domain name (FQDN) for the container
        # Type: list
        calibre_web_role_web_fqdn_override:
        ```

        !!! example "Example Override"

            ```yaml
            calibre_web_role_web_fqdn_override:
              - "{{ traefik_host }}"
              - "calibre_web2.{{ user.domain }}"
              - "calibre_web.otherdomain.tld"
            ```

            Note: Include `{{ traefik_host }}` to preserve the default FQDN alongside your custom entries


    ??? variable string "`calibre_web_role_web_host_override`"

        ```yaml
        # Override the Traefik web host configuration for the container
        # Type: string
        calibre_web_role_web_host_override:
        ```

        !!! example "Example Override"

            ```yaml
            calibre_web_role_web_host_override: "Host(`{{ traefik_host }}`) || Host(`{{ 'calibre_web2.' + user.domain }}`)"
            ```

            Note: Use `{{ traefik_host }}` to include the default host configuration in your custom rule


    ??? variable string "`calibre_web_role_web_http_port`"

        ```yaml
        # Type: string (quoted number)
        calibre_web_role_web_http_port:
        ```

    ??? variable string "`calibre_web_role_web_http_scheme`"

        ```yaml
        # Type: string ("http"/"https")
        calibre_web_role_web_http_scheme:
        ```

    ??? variable dict "`calibre_web_role_web_http_serverstransport`"

        ```yaml
        # Type: dict/omit
        calibre_web_role_web_http_serverstransport:
        ```

    ??? variable string "`calibre_web_role_web_scheme`"

        ```yaml
        # URL scheme to use for web access to the container
        # Type: string ("http"/"https")
        calibre_web_role_web_scheme:
        ```

    ??? variable dict "`calibre_web_role_web_serverstransport`"

        ```yaml
        # Type: dict/omit
        calibre_web_role_web_serverstransport:
        ```
<!-- END SALTBOX MANAGED VARIABLES SECTION -->
