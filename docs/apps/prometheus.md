...
Data retention defaults to 15 days but can be configured in your [Saltbox inventory](../saltbox/inventory/index.md) using `prometheus_role_retention` and `prometheus_role_size`.
...
Prometheus provides monitoring and alerting with automatic installation of Node Exporter and cAdvisor for system and container metrics. Configuration is at `/opt/prometheus/prometheus.yml`.
...
=== "Web"
    ...
    ??? variable string "`prometheus_role_web_url`"
        ...
        ```yaml
        # Type: string
        prometheus_role_web_url: "{{ lookup('role_web', role='prometheus', scheme='https') }}"
        ```
...
=== "Traefik"
    ...
    ??? variable string "`prometheus_role_traefik_middleware_default_api`"

        ```yaml
        # Type: string
        prometheus_role_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

    ??? variable string "`prometheus_role_traefik_middleware_custom_api`"

        ```yaml
        # Type: string
        prometheus_role_traefik_middleware_custom_api: ""
        ```
...
=== "Docker+"
    ...
    ??? variable bool "`prometheus_role_docker_gpu_enabled`"
        ...
        # Type: bool (true/false)
        prometheus_role_docker_gpu_enabled: false
        ```

    ??? variable bool "`prometheus_role_docker_nvidia_disabled`"
        ...
        # Type: bool (true/false)
        prometheus_role_docker_nvidia_disabled: false
        ```

    ??? variable bool "`prometheus_role_docker_dev_dri_disabled`"
        ...
        # Type: bool (true/false)
        prometheus_role_docker_dev_dri_disabled: false
        ```
    ...
    ??? variable string "`prometheus_role_docker_stop_signal`"

        ```yaml
        # Type: string
        prometheus_role_docker_stop_signal:
        ```
...
=== "Global Override Options"
    ...
    ??? variable string "`prometheus_role_web_api_http_port`"

        ```yaml
        # Type: string (quoted number)
        prometheus_role_web_api_http_port:
        ```

    ??? variable string "`prometheus_role_web_api_http_scheme`"

        ```yaml
        # Type: string ("http"/"https")
        prometheus_role_web_api_http_scheme:
        ```

    ??? variable dict "`prometheus_role_web_api_http_serverstransport`"

        ```yaml
        # Type: dict/omit
        prometheus_role_web_api_http_serverstransport:
        ```

    ??? variable string "`prometheus_role_web_api_port`"

        ```yaml
        # Type: string (quoted number)
        prometheus_role_web_api_port:
        ```

    ??? variable string "`prometheus_role_web_api_scheme`"

        ```yaml
        # Type: string ("http"/"https")
        prometheus_role_web_api_scheme:
        ```

    ??? variable dict "`prometheus_role_web_api_serverstransport`"
