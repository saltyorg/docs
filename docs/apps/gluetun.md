...
=== "Docker+"
    ...
    ??? variable bool "`gluetun_role_docker_gpu_enabled`{ .sb-show-on-unchecked }`gluetun2_docker_gpu_enabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        gluetun_role_docker_gpu_enabled: false
        ```

        ```yaml { .sb-show-on-checked }
        # Set this to true to let the app use a GPU. Intel access also requires gpu.intel: true; NVIDIA access also requires nvidia_enabled: true. This setting does not install or enable GPU support on the server.
        # Type: bool (true/false)
        gluetun2_docker_gpu_enabled: false
        ```

    ??? variable bool "`gluetun_role_docker_nvidia_disabled`{ .sb-show-on-unchecked }`gluetun2_docker_nvidia_disabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        gluetun_role_docker_nvidia_disabled: false
        ```

        ```yaml { .sb-show-on-checked }
        # Set this to true to turn off automatic NVIDIA access for this app. It only has an effect when the app's _docker_gpu_enabled option and nvidia_enabled are both true; automatic /dev/dri access may remain.
        # Type: bool (true/false)
        gluetun2_docker_nvidia_disabled: false
        ```

    ??? variable bool "`gluetun_role_docker_dev_dri_disabled`{ .sb-show-on-unchecked }`gluetun2_docker_dev_dri_disabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        gluetun_role_docker_dev_dri_disabled: false
        ```

        ```yaml { .sb-show-on-checked }
        # Set this to true to stop Saltbox from automatically sharing the server's /dev/dri video devices with this app. It only has an effect when the app's _docker_gpu_enabled option is true and either gpu.intel or nvidia_enabled is true; NVIDIA-specific access may remain.
        # Type: bool (true/false)
        gluetun2_docker_dev_dri_disabled: false
        ```
    ...
    ??? variable string "`gluetun_role_docker_stop_signal`{ .sb-show-on-unchecked }`gluetun2_docker_stop_signal`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        gluetun_role_docker_stop_signal:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        gluetun2_docker_stop_signal:
        ```
...
=== "Global Override Options"
    ...
    ??? variable list "`gluetun_role_docker_networks_alias_custom`{ .sb-show-on-unchecked }`gluetun2_docker_networks_alias_custom`{ .sb-show-on-checked }"
