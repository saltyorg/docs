...
=== "Web"
    ...
    ??? variable string "`node_red_role_web_url`{ .sb-show-on-unchecked }`node_red2_web_url`{ .sb-show-on-checked }"
        ...
        ```yaml { .sb-show-on-unchecked }
        # Type: string
        node_red_role_web_url: "{{ lookup('role_web', role='node_red', scheme='https') }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        node_red2_web_url: "{{ lookup('role_web', role='node_red', scheme='https') }}"
        ```
...
=== "Traefik"
    ...
    ??? variable string "`node_red_role_traefik_middleware_default_api`{ .sb-show-on-unchecked }`node_red2_traefik_middleware_default_api`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        node_red_role_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        node_red2_traefik_middleware_default_api: "{{ traefik_default_middleware_api }}"
        ```

    ??? variable string "`node_red_role_traefik_middleware_custom_api`{ .sb-show-on-unchecked }`node_red2_traefik_middleware_custom_api`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        node_red_role_traefik_middleware_custom_api: ""
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        node_red2_traefik_middleware_custom_api: ""
        ```
...
=== "Docker+"
    ...
    ??? variable bool "`node_red_role_docker_gpu_enabled`{ .sb-show-on-unchecked }`node_red2_docker_gpu_enabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        node_red_role_docker_gpu_enabled: false
        ```
        ...
        # This setting does not install or enable GPU support on the server.
        # Type: bool (true/false)
        node_red2_docker_gpu_enabled: false
        ```

    ??? variable bool "`node_red_role_docker_nvidia_disabled`{ .sb-show-on-unchecked }`node_red2_docker_nvidia_disabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        node_red_role_docker_nvidia_disabled: false
        ```
        ...
        # Automatic /dev/dri access may remain.
        # Type: bool (true/false)
        node_red2_docker_nvidia_disabled: false
        ```

    ??? variable bool "`node_red_role_docker_dev_dri_disabled`{ .sb-show-on-unchecked }`node_red2_docker_dev_dri_disabled`{ .sb-show-on-checked }"
        ...
        # Type: bool (true/false)
        node_red_role_docker_dev_dri_disabled: false
        ```
        ...
        # NVIDIA-specific access may remain.
        # Type: bool (true/false)
        node_red2_docker_dev_dri_disabled: false
        ```
...
=== "Global Override Options"
    ...
    ??? variable list "`node_red_role_paths_folders_list_custom`{ .sb-show-on-unchecked }`node_red2_paths_folders_list_custom`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Extra directories to create
        # Type: list
        node_red_role_paths_folders_list_custom:
        ```

        ```yaml { .sb-show-on-checked }
        # Extra directories to create
        # Type: list
        node_red2_paths_folders_list_custom:
        ```

    ??? variable string "`node_red_role_paths_group`{ .sb-show-on-unchecked }`node_red2_paths_group`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Group for directories created by the role
        # Type: string
        node_red_role_paths_group:
        ```

        ```yaml { .sb-show-on-checked }
        # Group for directories created by the role
        # Type: string
        node_red2_paths_group:
        ```

    ??? variable string "`node_red_role_paths_owner`{ .sb-show-on-unchecked }`node_red2_paths_owner`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Owner for directories created by the role
        # Type: string
        node_red_role_paths_owner:
        ```

        ```yaml { .sb-show-on-checked }
        # Owner for directories created by the role
        # Type: string
        node_red2_paths_owner:
        ```

    ??? variable string "`node_red_role_paths_permissions`{ .sb-show-on-unchecked }`node_red2_paths_permissions`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Permissions for directories created by the role
        # Type: string
        node_red_role_paths_permissions:
        ```

        ```yaml { .sb-show-on-checked }
        # Permissions for directories created by the role
        # Type: string
        node_red2_paths_permissions:
        ```

    ??? variable bool "`node_red_role_paths_recursive`{ .sb-show-on-unchecked }`node_red2_paths_recursive`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Apply owner and group recursively without changing child modes
        # Type: bool (true/false)
        node_red_role_paths_recursive:
        ```

        ```yaml { .sb-show-on-checked }
        # Apply owner and group recursively without changing child modes
        # Type: bool (true/false)
        node_red2_paths_recursive:
        ```
