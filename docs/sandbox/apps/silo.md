...
=== "Global Override Options"
    ...
    ??? variable list "`silo_role_paths_folders_list_custom`"

        ```yaml
        # Extra directories to create
        # Type: list
        silo_role_paths_folders_list_custom:
        ```

    ??? variable string "`silo_role_paths_group`"

        ```yaml
        # Group for directories created by the role
        # Type: string
        silo_role_paths_group:
        ```

    ??? variable string "`silo_role_paths_owner`"

        ```yaml
        # Owner for directories created by the role
        # Type: string
        silo_role_paths_owner:
        ```

    ??? variable string "`silo_role_paths_permissions`"

        ```yaml
        # Permissions for directories created by the role
        # Type: string
        silo_role_paths_permissions:
        ```

    ??? variable bool "`silo_role_paths_recursive`"

        ```yaml
        # Apply ownership and permissions recursively
        # Type: bool (true/false)
        silo_role_paths_recursive:
        ```
