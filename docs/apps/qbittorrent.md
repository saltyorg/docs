...
=== "Docker+"
    ...
    ??? variable string "`qbittorrent_role_docker_nvidia`{ .sb-show-on-unchecked }`qbittorrent2_docker_nvidia`{ .sb-show-on-checked }"

        ```yaml { .sb-show-on-unchecked }
        # Type: string
        qbittorrent_role_docker_nvidia:
        ```

        ```yaml { .sb-show-on-checked }
        # Type: string
        qbittorrent2_docker_nvidia:
        ```
